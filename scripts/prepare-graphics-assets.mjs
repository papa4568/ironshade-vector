...
function solarYardNodes(kind, lod) {
  ...
  if (kind === 'transferRail') {
    const nodes = [
      { name: 'solar-yard-transfer-rail-left', mesh: 1, translation: [0, 0.16, -0.74], scale: [3.62, 0.20, 0.16] },
      { name: 'solar-yard-transfer-rail-right', mesh: 1, translation: [0, 0.16, 0.74], scale: [3.62, 0.20, 0.16] },
      { name: 'solar-yard-transfer-rail-sleeper-left', mesh: 0, translation: [-2.34, 0.08, 0], scale: [0.30, 0.12, 1.94] },
      { name: 'solar-yard-transfer-rail-sleeper-center', mesh: 0, translation: [0, 0.08, 0], scale: [0.30, 0.12, 1.94] },
      { name: 'solar-yard-transfer-rail-sleeper-right', mesh: 0, translation: [2.34, 0.08, 0], scale: [0.30, 0.12, 1.94] },
      { name: 'solar-yard-transfer-rail-status', mesh: 4, translation: [2.86, 0.34, 0.96], scale: [0.22, 0.08, 0.08] },
    ];
    if (detail) {
      nodes.push(
        { name: 'solar-yard-transfer-rail-cable-channel', mesh: 2, translation: [0, 0.12, 1.10], scale: [3.20, 0.10, 0.12] },
        { name: 'solar-yard-transfer-rail-gold-junction-left', mesh: 3, translation: [-1.20, 0.28, 0], scale: [0.26, 0.08, 0.96] },
        { name: 'solar-yard-transfer-rail-gold-junction-right', mesh: 3, translation: [1.20, 0.28, 0], scale: [0.26, 0.08, 0.96] },
      );
    }
    nodes.push({ name: 'environment-root', children: nodes.map((_, index) => index) });
    return nodes;
  ...
  if (kind === 'gantryCrane') {
    const nodes = [
      ...
      { name: 'solar-yard-gantry-crane-beam', mesh: 1, translation: [0, 4.02, 0], scale: [3.86, 0.42, 5.46] },
    ...
    if (detail) {
      ...
      nodes.push({ name: 'solar-yard-gantry-crane-cable-run', mesh: 2, translation: [0, 4.28, 0], scale: [3.12, 0.10, 0.12] });
