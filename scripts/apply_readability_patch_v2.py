...
def regex_once(path: str, pattern: str, replacement: str) -> None:
    ...
    if count != 1:
        raise SystemExit(f"{path}: regex expected one match for {pattern[:120]!r}")
    write(path, next_text)


# Global readability styles.
replace_once('src/App.tsx', "import './equipmentBay.css';", "import './equipmentBay.css';\nimport './readability.css';")
replace_once('src/App.tsx', "import { recoveryQualityLabel } from './game/lootQuality';\n", "")
...
regex_once(
    ...
    r"function FactionSetOverview\(\{ profile \}: \{ profile: PlayerProfile \}\) \{.*?\n\}\n\n(?=function effectiveWeapon)",
    "",
)
replace_once(
    'src/components/Armory.tsx',
...
replace_once('src/components/Armory.tsx', "  const [showDoctrines, setShowDoctrines] = useState(false);\n", "")
replace_once('src/components/Armory.tsx', "  const activeDoctrineCount = doctrineStates.filter(state => state.twoPieceActive).length;\n", "  const activeDoctrineStates = doctrineStates.filter(state => state.twoPieceActive);\n")
replace_once(
    'src/components/Armory.tsx',
...
regex_once(
    'src/components/Armory.tsx',
    r"\{tab === 'gear' && <><section className=\"gear-overview-bar\">.*?</section>\{showDoctrines && <FactionSetOverview profile=\{profile\} />\}<div className=\{`gear-layout \$\{selected \? 'has-selection' : ''\}`\}>",
...
replace_once('src/components/GameCanvas.tsx', "import { lootColor, type GroundLootReceipt } from '../game/fieldLoot';", "import { lootColor, lootLabel, type GroundLootReceipt } from '../game/fieldLoot';")
replace_once(
...
replace_count('src/game/threeCombatRenderer.ts', "const barWidth = enemy.role === 'boss' ? 1.8 : 1.05;", "const barWidth = enemy.role === 'boss' ? 2.3 : enemy.role === 'elite' ? 1.6 : 1.35;", 2)
...
replace_once('src/game/threeCombatRenderer.ts', "      visual.hp.scale.x = hpRatio;", "      visual.barRoot.scale.setScalar(enemy.maxArmor > 0 && enemy.armor <= 0 ? 1.12 : 1);\n      visual.hp.scale.x = hpRatio;")

# Add a focused regression gate for the information hierarchy.
replace_once('package.json', '"build": "npm run test:beta && npm run test:gameplay && npm run test:maps && npm run test:loot && vite build",', '"build": "npm run test:beta && npm run test:gameplay && npm run test:maps && npm run test:loot && npm run test:ui && vite build",')
replace_once('package.json', '"test:loot": "vite build --ssr tests/loot-difficulty.ts --outDir .loot-dist && node .loot-dist/loot-difficulty.js"', '"test:loot": "vite build --ssr tests/loot-difficulty.ts --outDir .loot-dist && node .loot-dist/loot-difficulty.js",\n    "test:ui": "vite build --ssr tests/ui-readability.ts --outDir .ui-dist && node .ui-dist/ui-readability.js"')

print('readability patch v2 applied')
