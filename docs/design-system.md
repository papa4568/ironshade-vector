...
- **Guide ownership:** durable “how it works,” control reference, system teaching, rarity/equipment explanation, and other reusable instructional copy should have one canonical home in Intel → Guide. Feature screens may deep-link to the relevant Guide section and may retain short contextual hints, but do not duplicate long instructional bodies across surfaces.
- **Local decision information stays local:** moving instruction into Guide must never move an immediate cost, tradeoff, warning, eligibility blocker, blocker reason, next requirement, combat tell, or transient onboarding cue away from the action/context where the player needs it.
