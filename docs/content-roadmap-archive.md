...
- [x] **P9-C Build-defining wave** — expanded the Deep Operator Network from 69 to 93 authored nodes with a complete build-defining layer: one Mastery, two mutually exclusive Keystones, and one Capstone for each of Ballistics, Mobility, Systems, Survival, Engineering, and Awareness.
  - Masteries extend each outer branch with a focused payoff and open the branch decision. Keystones cost 2 points and commit to explicit upside/downside mechanics such as penetration versus recoil/heat, armor damage versus health damage, movement versus armor, ability efficiency versus cooldown, defense versus speed, thermal aggression versus heat, and precision/range versus reload or raw damage. Capstones cost 2 points and reward the committed branch direction.
  - Added typed `exclusiveGroup` ownership so a branch cannot allocate or path-preview both Keystone alternatives. Existing class starts, off-class weapon-sector locks, six-link outer ring, Network Schema 1 saves, legacy mirrors, and point normalization remain unchanged.
  - Runtime Operator Network effects now also support weapon heat-per-shot and direct-health multipliers so Keystone tradeoffs are mechanical rather than presentation-only. The Progression UI identifies Mastery/Keystone/Capstone nodes directly and reports alternative Keystone commitments without adding a gear-score-style summary.
  - Extended `tests/operator-network-architecture.ts` to cover 93 nodes / 105 edges, 6 Masteries, 12 Keystones, 6 Capstones, per-branch structure, mutual exclusion, Capstone route cost, and live combat-value tradeoffs. PR #160 Browser E2E `35683000757` passed desktop and mobile-landscape before merge.
  - Verified on merged main `1ad3f866b05983897ed943ab8668b2e4cc4c614a`: Browser E2E `35683161325`, Level 15 beta smoke `35683161312`, and Android beta.249 `35683161327` all passed. Production output included `OPERATOR_NETWORK_ARCHITECTURE_PASS schema=1 nodes=93 edges=105 outer=6 starts=3 coreWave=36 classWeapon=12 buildDefining=24` and save migration remained green.
  - Android verification passed full web regression/build, native project generation, package/version/SDK/signature verification, emulator install/runtime/touch/lifecycle smoke, authored-content verification, and Chapter 3 playthrough. Artifact `10675363272` contains `Ironshade-Vector-Android-Beta.apk`, package `app.ironshade.vector`, version `0.0.1-beta.249`, debug signing, and APK SHA-256 `30375484c164394ddc92bf9d1a6ef8ef167ddd09c0bf9c3d10e6b0cbca399f60`.
