...
## 2026-09-23 — P15-E AAA UI / World / Cinematic Polish // Accessibility/mobile/screenshot gate

- [x] **P15-E Accessibility/mobile/screenshot gate** — completed the final P15 presentation gate with persistent scalable text, high contrast, reduced motion, existing effect/audio/assist controls, safe-area/orientation coverage, and screenshot/readability QA.
  - Extended the save-safe operator profile settings with `textScale`, `contrast`, and `reducedMotion`; old saves inherit defaults through the existing settings merge without a profile reset. Build → Settings exposes Large text, High contrast, and Reduce motion alongside the established touch aim assist, assisted fire tracking, screen shake, effect intensity, effects/UI volume, haptics, and telemetry controls.
  - Applied persisted presentation preferences at the document root. Large text scales rem-based UI to 112.5%; high-contrast mode overrides shared P15 surface/text/border/focus tokens; the production regression verifies primary and secondary text exceed a 7:1 contrast target on raised surfaces.
  - Reduced motion disables CSS animation/transitions independently of `prefers-reduced-motion` and also suppresses combat camera shake/target-feedback motion without changing simulation timing. Existing Reduced Effects remains an independent VFX-quality control.
  - Added explicit accessible names to every Settings select/checkbox/range after the live audit exposed six legacy controls that depended only on wrapping-label text. The final live accessibility audit passes with the complete form.
  - Added `tests/accessibility-mobile-gate.ts` / `test:accessibility-mobile` to the production build. The gate locks persistence, global presentation wiring, audio/haptic/assist integration, four-sided safe-area variables, desktop + mobile-landscape browser coverage, Android sensor-landscape coverage, screenshot retention, and Android mobile/lifecycle smoke requirements.
  - Browser runtime QA now drives Large text + High contrast + Reduce motion through the real Settings UI, verifies atomic profile persistence, checks horizontal fit/root text scaling/contrast/reduced-motion state, runs the accessibility audit, and retains `browser-accessibility-<viewport>.png`. Final mobile screenshot review confirmed the 851×360 Settings surface remains readable and horizontally contained with Large text + High contrast active.
  - PR #209 Browser E2E run `35870108712` passed the full desktop + mobile-landscape regression/production build and live player journeys after CI first caught an outdated target-motion regression assertion and then the six legacy unnamed Settings controls; both were fixed without weakening the gate.
  - Merged source `04e37a3e49cb5894cfbd6fc5c9cf017dc5714786` passed post-merge Browser E2E `35870541538` (run 665, desktop + mobile-landscape) and Level 15 beta smoke `35870541561` (run 530).
  - Android beta.298 run `35870541607` passed the full web regression/build, native project generation, debug APK build, package/version/SDK/signature verification, emulator install/cold launch, class/menu/skill/mission presentation, mobile layout/touch/runtime/lifecycle QA, authored-content checks, and Chapter 3 touch playthrough. Runtime markers included `ANDROID_MOBILE_LAYOUT_PASS viewport=851x412`, `ANDROID_LIFECYCLE_RESUME_PASS`, and `ANDROID_EMULATOR_PASS`.
  - Android artifact `10754752584` contains `Ironshade-Vector-Android-Beta.apk`, package `app.ironshade.vector`, version `0.0.1-beta.298` (298), debug signing, compile/target SDK 36 verification, and APK SHA-256 `c8420ac820241c8ce3c01fdbb8d5be68b7ec3891170bab726f714daa20cf0dba`.
  - **Next: P16-A — Measurement.**

## 2026-09-23 — P16-A Performance Without Compromise // Measurement

- [x] **P16-A Measurement** — established explicit device tiers and separate CPU/render/GPU/UI/animation/audio/GC budgets with deterministic regression detection and reproducible runtime diagnostics.
  - Added `src/game/performanceDiagnostics.ts` with versioned `p16-a-v1` budgets for flagship, midrange, and minimum tiers. All tiers target 60 fps frame pacing while maintaining separate p95 CPU, render, UI, animation, and audio limits; GPU draw-call/triangle workload limits; and GC heap/collection-rate limits.
  - Device-tier selection is deterministic from hardware concurrency/device-memory hints and supports an explicit `perfTier` QA override. The measurement layer does not change gameplay or quality behavior.
  - GameCanvas now samples simulation CPU, renderer cost, UI telemetry work, presentation animation work, feedback/audio dispatch, JS heap/collection observations, and WebGL renderer workload. The active canvas exposes the budget version, device tier, scenario, sample count, classification status, regressions, and a serialized diagnostic report.
  - Three.js exposes draw-call and triangle counts through its existing renderer info so GPU workload is measured without introducing a new dependency or changing render behavior.
  - Added `tests/performance-measurement.ts` / `test:performance-measurement` to the production build. The deterministic gate runs comparable 180-sample representative baselines for all three device tiers and independently injects CPU, render, GPU, UI, animation, audio, and GC regressions to prove each category is identified against its own explicit budget.
  - Browser runtime QA captures versioned refinery performance reports for desktop and mobile-landscape. The final main Browser E2E run `35876383223` (run 673) passed both viewports and retained artifacts `10757972354` (desktop) and `10758995111` (mobile-landscape). The live reports expose the same category schema and budget version; software-rendered CI captures after 12+ observable frames while the diagnostics engine retains a 45-sample window before classifying budget status.
  - PR #210 established the measurement system. Its first CI attempt exposed that a 90-frame live wait was inappropriate on software-rendered CI and could delay unrelated controller QA; the baseline collection was scoped to the representative refinery flow and the engine classification window was reduced to 45 samples. PR #211 then removed the remaining runner-speed flake by decoupling artifact capture from the 45-sample classification threshold without weakening deterministic regression detection.
  - Final main source `165bc442f3a9d26e249091eaf46ff8ee0ff775ed` passed Browser E2E `35876383223` (run 673), Level 15 beta smoke `35876383461` (run 535), and Android beta.300 `35876383267`.
  - Android beta.300 passed the full production regression/build, native project generation, debug APK build, package/version/min-SDK/target-SDK/signature verification, emulator install/cold launch, mobile layout/touch/runtime/lifecycle QA, authored-content checks, and Chapter 3 touch playthrough. Runtime markers included `ANDROID_MOBILE_LAYOUT_PASS viewport=851x412`, `ANDROID_LIFECYCLE_RESUME_PASS`, `ANDROID_CHAPTER3_PLAYTHROUGH_PASS`, and `ANDROID_EMULATOR_PASS`.
...
## 2026-09-23 — P16-C Performance Without Compromise // Runtime scalability

- [x] **P16-C Runtime scalability** — bounded runtime allocation/work pressure across pooled visuals, authored animation, combat audio, and graphics streaming/preload without changing encounter mechanics or gameplay-critical combat cues.
  - Added `src/game/runtimeScalability.ts` with High/Balanced/Performance runtime profiles for retained idle pool capacity, distance-aware authored-enemy animation cadence, bounded preload concurrency/residency, and startup preload deferral. The performance tier keeps the smallest retained pools and serialized preload work while retaining readable damage-number capacity.
  - Burst-grown damage-number, secondary-effect, impact-spark, and debris pools now trim back only after hysteresis thresholds are exceeded and never below the active visual count, avoiding both permanent burst residency and per-frame allocation churn.
  - Authored enemy rig work now uses distance/tier-aware animation LOD. Bosses, targeted enemies, near enemies, telegraphs, lifecycle transitions, active statuses, impact/armor-break reactions, and other critical cues remain full-cadence; only distant noncritical standard/elite rigs reduce update cadence.
  - Combat audio now virtualizes lower-priority background/normal voices before important/critical cues consume the main/reserved voice ceilings. Runtime telemetry reports active/tail/virtualized voices and the last virtualization reason while preserving the existing priority mix and critical reserve.
  - Added bounded-concurrency `preloadGraphicsAssets` to the graphics asset runtime and mission-aware preload selection in the Three.js renderer. Mobile/performance preload is serialized and noncritical preload is deferred through first-playable startup, then transitions on renderer wall-clock time so it cannot remain blocked by paused/idle simulation time.
  - Added `tests/runtime-scalability.ts` / `test:runtime-scalability` to the production build and Android runtime telemetry for animation LOD, pool retention, deferred-to-active asset streaming, and audio virtualization. The deterministic gate also locks boss/target/critical-cue full cadence and active-pool safety.
  - Verification found and fixed three integration issues without weakening the gate: the initial preload competed with Android first-playable/deployment QA; the Android assertion initially treated intentional startup deferral as failure; and simulation-time deferral could remain paused under the field coach. PRs #213–#215 progressively deferred preload, verified the deferred-to-active transition, and moved the delay to renderer wall-clock time.
  - Final implementation source `97dadd8e2c42bb2f9a9acfe5c3eada35d2f70669` passed post-merge Browser E2E `35891882312` (run 688, desktop + mobile-landscape) and Level 15 beta smoke `35891882343` (run 548).
  - Android beta.308 run `35891882306` passed the full web regression/build, native project generation, debug APK build, package/version/min-SDK/target-SDK/signature verification, emulator install/cold launch, runtime scalability telemetry, mobile/touch/runtime/lifecycle QA, authored-content checks, and Chapter 3 touch playthrough. Runtime markers included `ANDROID_RUNTIME_SCALABILITY_PASS ... assets=bounded-preload:6@1`, `ANDROID_MOBILE_LAYOUT_PASS viewport=851x412`, `ANDROID_LIFECYCLE_RESUME_PASS`, `ANDROID_CHAPTER3_PLAYTHROUGH_PASS`, and `ANDROID_EMULATOR_PASS`.
  - Android artifact `10765023772` contains `Ironshade-Vector-Android-Beta.apk`, package `app.ironshade.vector`, version `0.0.1-beta.308` (308), debug signing, min SDK 24 / target SDK 36, and APK SHA-256 `1bca099e1107aa308635d602306e027773c992fa70b30f868d92a9107a38d15d`.
  - **Next: P16-D — Boot architecture.**

## 2026-09-23 — P16-D Performance Without Compromise // Boot architecture

- [x] **P16-D Boot architecture** — staged startup work behind explicit module boundaries and added versioned boot-cost diagnostics without introducing arbitrary bundle/chunk byte caps.
  - `src/main.tsx` now renders a lightweight boot surface immediately, dynamically loads save recovery, waits for recovery to complete, and only then dynamically loads the full `App`. Blocked/corrupt recovery still stops startup before the app can overwrite existing save data.
  - Added `src/game/bootDiagnostics.ts` with versioned `p16-d-v1` timing for entry, recovery module readiness, recovery completion, app module readiness, render request, first frame, blocked recovery, and unexpected startup failure. The latest report is published through document data attributes and a debug window hook for reproducible QA.
  - Added `tests/boot-architecture.ts` / `test:boot-architecture` to the production build. The deterministic gate verifies stage ordering, independent duration tracking, total timing, and immutable report snapshots.
  - Extended `tests/client-bundle-architecture.mjs` so production builds must emit separate `saveRecovery-*` and `App-*` chunks, retain their dynamic imports in the boot source, and keep them out of the synchronous boot graph. The architecture gate remains intentionally byte-cap-free.
  - Verification caught three assertion-shape issues in the new bundle test while the application itself continued producing the intended deferred chunks: Vite does not guarantee source-path dynamic-entry keys, sequential dynamic imports are not both listed as direct HTML-entry dynamic imports, and type-only imports must not count as runtime imports. Each was corrected without weakening the architectural requirement.
  - PR #217 final head `7f56efea575b89e82a5a0d0daefd6630da2ded13` passed Browser E2E run `35894433185` (run 692) on desktop + mobile-landscape. Merged source `22fae5827e226b939bc7aa1825e814f3fb2a6c6e` passed post-merge Browser E2E `35894842124` (run 693) and Level 15 beta smoke `35894842117` (run 552).
...
## 2026-09-23 — P18-A Beta Combat Reliability & UI Clarity // Assisted targeting + Spin Habitat fire reliability

- [x] **P18-A — Assisted targeting + Spin Habitat fire reliability** — completed and verified smooth assisted FIRE execution plus deterministic Spin Habitat transition coverage.
  - Assisted FIRE no longer assigns a hard target vector at shot time. It now requires the retained target id to remain legal, visible, within weapon acquisition range, and within a 10° convergence window; otherwise the shot is withheld without mutating operator aim.
  - Retained target control now rejects bosses while the deep-zone boss gate is held or the boss is unavailable, and the combat loop explicitly clears any retained lock at the boss-gate checkpoint. Existing death, range-loss, and occlusion-grace invalidation remain in the shared target-control path; manual FIRE behavior remains unchanged.
  - Added deterministic gameplay coverage for no-snap convergence, blocked fire while unconverged or occluded, stale boss-id rejection, unchanged manual aim, projectile creation/damage after convergence, and a Spin Habitat rim/spoke/axis route exercise across the 10s emergency spindown and 22s gravity recovery before and after boss availability.
  - Verification found two QA-only issues and fixed them without weakening gameplay gates: the first route assertion incorrectly required simplified navigation waypoints inside every crossed sector, so it now checks route-segment crossings; Android CI could reclaim the app process during the pause/resume smoke, so the harness re-establishes combat once after an incidental reclaim and still requires a preserved-process pause/resume to pass. The UI source-contract regression was updated to require that stronger lifecycle behavior.
  - Final source `623b5ac7cb9fd705519e7f4bb26ccbca6b2b0f12` passed Level 15 beta smoke `35927945988` and Browser E2E `35927945960` on desktop + mobile-landscape. The gameplay gate reported `GAMEPLAY_REGRESSIONS_PASS`; map coverage reported `MAP_NAVIGATION_PASS locations=11 objectives=23 minimumReachable=100.0%`; the production build and bundle/runtime asset checks passed.
  - Android beta.323 run `35927945921` passed the repeated full web regression/build, native project generation, debug APK build, package/version/min-SDK/target-SDK/signature verification, emulator install/cold launch, touch combat smoke, preserved-process lifecycle resume, authored combat rendering, and Chapter 3 touch playthrough. Runtime evidence included `ANDROID_TOUCH_SMOKE_PASS move=drag aim=drag fire=hold ability=tap dodge=tap`, `ANDROID_RUNTIME_SMOKE_PASS`, `ANDROID_LIFECYCLE_RESUME_PASS process=preserved`, `ANDROID_CHAPTER3_PLAYTHROUGH_PASS`, and `ANDROID_EMULATOR_PASS`.
  - Android artifact `10780247068` contains `Ironshade-Vector-Android-Beta.apk`, package `app.ironshade.vector`, version `0.0.1-beta.323` (323), debug signing, min SDK 24 / target SDK 36, and APK SHA-256 `c3c73776556bdf0b085eb3ba14ef6f71b6deae67d96ebdd46b177201e7e62793`.
  - **Next: P18-B — Attack Speed owns tracking + firing cadence.**

## 2026-09-24 — P18-E Beta Combat Reliability & UI Clarity // Gear inspector quick read

- [x] **P18-E — Gear inspector quick read** — converted item inspection to a decision-first equipment flow while preserving the full expert data set behind the shared Details sheet.
  - The default inspector now leads with the shared requirement-state presentation for ready, active, class-family blocked, and level-blocked states, including plain-language blocker reasons and the next requirement. Item identity/rarity remains in the inspector header and the primary equip/unequip action remains in the fixed action dock.
  - The quick read keeps the primary effect, a compact current-vs-candidate comparison of the most decision-relevant stats, and genuinely build-changing Singular/mechanical/specialization rules visible without opening secondary help.
  - Explicit modifiers, Augments, full stat comparison, generated build links, frame/recovery provenance, registry scopes/tags, compatibility telemetry, and Singular identity detail now live in the reusable progressive-disclosure Details sheet with labeled sections; no expert data was removed.
  - Added focused gear/readability/menu regression coverage and Android runtime smoke coverage for an off-class stored weapon comparison, explicit class-family blocker/reason/next step, decision-first viewport placement, opening/closing the Details sheet, and a real Combat Suit unequip/re-equip cycle that restores the original profile state.
  - Verification found two stale regression assertions that encoded the former always-expanded hierarchy. They were updated to preserve their information-availability contracts while accepting the P18-E progressive-disclosure hierarchy; no application requirement was weakened.
  - PR #225 final head `748eabe0339bf5f8af350b01452fcdf3b6fc23ff` passed Browser E2E `36011112225` on desktop and mobile-landscape, including the full regression suite, production build, and live browser journey.
  - Merged source `5db6042616f878b51c7dcacc24c560c4d8433210` passed post-merge Browser E2E `36011491496` (run 724), Level 15 beta smoke `36011491356` (run 590), and Android beta.331 `36011491476`.
  - Android beta.331 passed the repeated full web regression/production build, native project generation, debug APK build, package/version/min-SDK/target-SDK/signature verification, API 35 emulator install/cold launch, `ANDROID_P18_GEAR_INSPECTOR_PASS`, disclosure/requirement-state checks, touch/controller combat QA, preserved-process lifecycle resume, Chapter 3 touch playthrough, cold-relaunch planner persistence, and `ANDROID_EMULATOR_PASS`.
...
## 2026-09-24 — P19-A Glance-First Adaptive UI // Combat HUD information layers + mobile type floor

- [x] **P19-A — Combat HUD information layers + mobile type floor** — recomposed compact landscape combat around persistent core information, contextual state, and a single transient-alert lane while preserving combat controls, critical cues, assisted targeting, accessibility behavior, and safe areas.
  - Kept HP/armor/capacitor, objective state, and the right-thumb action dock continuously available; target/boss, loot, interaction, optional/post-clear, class-mechanic, and related context surfaces remain state-driven rather than permanently competing for space.
  - Routed deployment, boss transition, tactical event, status, and tutorial messaging through the shared compact-phone transient lane with explicit priority instead of stacking independent alert surfaces.
  - Added the compact combat type floor and overlap policy in the Glance-First HUD styles: secondary live combat text is held to at least 12px, decision text to the 14–16px class where applicable, and critical numbers/actions remain larger; mobile regression coverage rejects undersized live text and core/context/transient overlap.
  - Android verification exposed two stale smoke assumptions that depended on visible FIELD COACH copy even while higher-priority transient content correctly occupied the shared lane. PRs #228 and #229 updated the verifier to recognize the shared lane and observe tutorial/input progression independently of transient presentation priority without weakening movement, manual-aim, FIRE, ability, dodge, controller, or HUD-overlap checks.
  - PR #229 head `02efee0b063cb4a47fe6a9bc2792c99e699aab2b` passed Browser E2E run `36038090994` (run 737) on desktop + mobile-landscape. Final merged source `f3d0132f7c42f710f0f324054f9044642e34af3f` passed post-merge Browser E2E run 738 and Level 15 beta smoke run 607.
  - Android beta.340 run `36038562979` passed the repeated full web regression/production build, native Android generation, debug APK build, package/version/min-SDK/target-SDK/signature verification, API 35 emulator install/cold launch, compact combat layout checks, touch combat, controller targeting, lifecycle resume, authored-runtime checks, Chapter 3 touch playthrough, cold-relaunch planner persistence, and `ANDROID_EMULATOR_PASS`.
  - Native P19-A evidence included `ANDROID_MOBILE_LAYOUT_PASS viewport=851x412 touchButtons=6 safe=onscreen+separated`, `ANDROID_COMBAT_HUD_PASS layers=core+context+transient typeFloor=12px overlaps=none touch=clear`, `ANDROID_CONTROLLER_TARGETING_SMOKE_PASS target=1 trigger=R2 acquisition=assisted fire=confirmed`, `ANDROID_TOUCH_SMOKE_PASS move=drag aim=drag fire=hold ability=tap dodge=tap weapon=class-locked scroll=0,0`, and `ANDROID_RUNTIME_SMOKE_PASS`.
  - Android artifact `10825477987` contains `Ironshade-Vector-Android-Beta.apk`, package `app.ironshade.vector`, version `0.0.1-beta.340` (340), debug signing, min SDK 24 / target SDK 36, and APK SHA-256 `d83d4ec0d38c3e8d3d74efbd0ac9b37e73d3fdcae601e9a57e9b2e56debf8eab`.
  - **Next: P19-B — Adaptive five-item Command dock.**


## 2026-09-24 — P19-B Glance-First Adaptive UI // Adaptive five-item Command dock

- [x] **P19-B — Adaptive five-item Command dock** — kept the existing Command / Operations / Operator / Ship / Intel destination model while recomposing primary navigation into a safe-area-aware five-item bottom dock on compact phone windows and preserving the rail on wider layouts.
  - Implementation commit `434dafbb470ad28de2cea3be0a66bc6c16e63938` added the shared five-destination keyboard/controller/touch routing, compact dock composition, 52px-class targets, 12px compact labels, active-page semantics, safe-area handling, wide-rail breakpoint behavior, browser coverage, and native Android breakpoint/rotation/controller checks. Follow-up `e9bfb7a2b110c59632197dc579b7722bcdf95549` aligned the UI-readability regression with the new P19-B coverage.
  - Verification exposed a legacy smoke assumption that tried to activate the intentionally hidden compact-phone Equipment rail shortcut. Commits `e70dff1b9ec98c8e0694f02b428377ea3aff7073` and `14025b77ea42c0c5eb963802bf1683a2f1044f83` kept the five-item primary dock unchanged and updated compact browser/Android Build access verification to follow the retained Operator → Build route while wide layouts continue to exercise the Equipment shortcut.
  - Final source `14025b77ea42c0c5eb963802bf1683a2f1044f83` passed Level 15 beta smoke run `36066826698` and Browser E2E run `36066826773` on desktop + mobile-landscape. Mobile evidence included `BROWSER_P19_COMMAND_NAV_PASS viewport=mobile-landscape composition=dock destinations=5 target=52px label=12.0px safe=simulated`, `BROWSER_P19_COMMAND_INPUT_PASS keyboard=arrows+space+escape controller=dpad+a+b routing=shared`, and the full menu/player journey.
  - Android beta.353 run `36066826640` passed the repeated full web regression/production build, native Android generation, debug APK build, package/version/min-SDK/target-SDK/signature verification, API 35 emulator install/cold launch, lifecycle/persistence checks, and `ANDROID_EMULATOR_PASS`.
  - Native P19-B evidence included `ANDROID_P19_COMMAND_NAV_PASS destinations=5 touch=command+intel controller=dpad+a+b back=controller-b breakpoint=dock+rail rotation=portrait+landscape native=851x412`, confirming compact/wide breakpoint behavior, touch + controller routing, back behavior, and portrait/landscape rotation without horizontal content loss.
  - Android artifact `10836792396` contains `Ironshade-Vector-Android-Beta.apk`, package `app.ironshade.vector`, version `0.0.1-beta.353` (353), debug signing, min SDK 24 / target SDK 36, and APK SHA-256 `a763182f40aac04cf937c5edd911c7e40e5e28fe4b55b54177f6ca3a820eeef0`.
  - **Next: P19-C — Intel Guide tab + canonical how-to content.**

## 2026-09-25 — P19-C Glance-First Adaptive UI // Intel Guide tab + canonical how-to content

- [x] **P19-C — Intel Guide tab + canonical how-to content** — added Guide as a secondary Intel destination, centralized durable system teaching into a data-driven canonical content model, and replaced migrated long-form feature-screen teaching with contextual Guide links while retaining immediate costs, tradeoffs, warnings, eligibility states, blocker reasons, and next requirements at the point of action.
  - Implementation began at `0b09d59b2d755ae084c98b1d2707a0f7f2d5bfca` with the six-section Guide model (Combat & Controls, Equipment & Rarity, Builds & Progression, Crafting, Ship & Operations, Accessibility / Settings), responsive Guide presentation, Intel secondary-tab routing, Build-to-Guide deep links, history/back handling, and focus restoration. Follow-up commits completed the crafting/rarity/arsenal ownership migration and aligned affected regressions with canonical Guide ownership instead of reintroducing duplicate instructional bodies.
  - Verification found two stale UI-readability expectations and an ultra-short Android landscape fit failure before the Guide smoke could run. Commits `a9aa26df016a1d50b5e322c01208a578c5ff2e0a` and `86b7587b0de12d1114283884af0ea58bf9189c45` updated the stale Intel/arsenal assertions without weakening behavior checks; final layout commit `e0d02e3d90c8033afa532fffa07b565bbbf3b4fb` reclaimed Command header/padding space under 380px landscape height while keeping all five primary destinations and actions intact.
  - Final source `e0d02e3d90c8033afa532fffa07b565bbbf3b4fb` passed Level 15 beta smoke run `36100447187` (full production build) and Browser E2E run `36100447163` on desktop + mobile-landscape.
  - Android beta.361 run `36100447229` passed the repeated full web regression/production build, native Android generation, debug APK build, package/version/min-SDK/target-SDK/signature verification, API 35 emulator install/cold launch, runtime/player-journey smoke, and QA artifact upload.
