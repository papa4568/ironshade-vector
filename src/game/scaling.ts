...
export function applyThreatBudget(enemies: Enemy[], contract: Contract) {
  ...
  for (const enemy of enemies) { enemy.protocols = []; enemy.mutations = []; enemy.commandTargetMutations = []; enemy.commandMutationCooldown = 0; enemy.bossPhaseMutations = []; enemy.bossMutationCooldown = 0; enemy.protocolPulse = 0; enemy.combatClass = enemy.role === 'boss' ? 'command' : enemy.role === 'elite' ? 'elite' : 'standard'; }
  ...
  if (boss) { const bossEffectiveness = Math.max(1, 1 + (effectiveness - 1) * 0.82); boss.effectiveness = bossEffectiveness; boss.maxHp = Math.max(1, Math.round(boss.maxHp * bossEffectiveness)); boss.hp = boss.maxHp; boss.maxArmor = Math.max(0, Math.round(boss.maxArmor * bossEffectiveness)); boss.armor = boss.maxArmor; const commandTargetMutations = chooseCommandTargetMutations(contract); applyCommandTargetMutations(boss, commandTargetMutations); boss.commandMutationCooldown = commandTargetInitialCooldown(commandTargetMutations); boss.bossPhaseMutations = chooseBossPhaseMutations(contract); }
