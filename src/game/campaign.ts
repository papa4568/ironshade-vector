...
export type ObjectiveMode = 'pressure-recovery' | 'grid-isolation' | 'gravity-stabilization' | 'machinery-recovery' | 'emergency-boarding' | 'deep-salvage' | 'momentum-capture' | 'thermal-routing';
export type LocationId = 'orbital-station' | 'damaged-vessel' | 'asteroid-refinery' | 'spin-habitat' | 'jovian-harvester' | 'ice-mine' | 'solar-yard' | 'lattice-annex' | 'momentum-exchange' | 'cryo-reserve';
...
export type StoryState = { arcs: Record<StoryArcId, StoryArcProgress>; latticeClues: number; lastBeat: string; blackLattice: BlackLatticeProgress; postKhepri: PostKhepriProgress; interdiction: InterdictionProgress };
...
export function locationNameFor(id: LocationId) { return locations.find(location => location.id === id)?.name ?? (id === 'lattice-annex' ? 'Khepri Survey Annex' : id === 'momentum-exchange' ? 'Cislunar Momentum Exchange' : id === 'cryo-reserve' ? 'Umbra Cryogenic Propellant Reserve' : id); }
...
export function getMegastructureStageContract(contract: Contract, requestedStage: number): Contract {
  ...
  return {
    ...
    directorPreview: `MEGASTRUCTURE SPACE ${stageIndex + 1}/${definition.stages.length} // ${tacticalIdentityForLocation(stage.location).forecast || 'Director timing remains fixed and disclosed.'} Internal extraction becomes available after this space is secured.`,
