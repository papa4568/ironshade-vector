...
export function modifierCountForRarity(rarity: RecoverableRarity, quality: RecoveryQualityGrade, random: () => number) { if (rarity === 'Field') return quality >= 2 || random() < 0.58 ? 1 : 0; if (rarity === 'Refined') return 2 + (quality >= 2 && random() < 0.55 ? 1 : 0); return 4 + (quality >= 4 && random() < 0.55 ? 1 : 0); }
