...
export function modifierCountForRarity(rarity: RecoverableRarity, quality: RecoveryQualityGrade, random: () => number) { if (rarity === 'Field') return 0; if (rarity === 'Refined') return random() < Math.min(0.88, 0.58 + quality * 0.06) ? 2 : 1; const roll = random(); if (roll < 1 / 12) return 6; if (roll < 4 / 12) return 5; return 4; }
