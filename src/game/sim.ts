...
export type Player = { x: number; y: number; vx: number; vy: number; aim: Vec2; move: Vec2; hp: number; maxHp: number; armor: number; maxArmor: number; capacitor: number; maxCapacitor: number; fireCooldown: number; abilityCooldowns: [number, number, number]; dodgeCooldown: number; dodgeTime: number; invulnerable: number; consumableCooldown: number; weaponHeat: Record<WeaponId, number>; mags: Record<WeaponId, number>; reloadT: number; reloadWeapon: WeaponId; ventT: number; dead: boolean; currentWeapon: WeaponId; vacuumExposure: number; disrupted: number };
...
function stepPlayer(state: SimState, dt: number) {
  const p = state.player; const sector = currentSector(state, p.x, p.y); p.fireCooldown = Math.max(0, p.fireCooldown - dt); p.abilityCooldowns = p.abilityCooldowns.map(value => Math.max(0, value - dt)) as [number, number, number]; p.dodgeCooldown = Math.max(0, p.dodgeCooldown - dt); p.dodgeTime = Math.max(0, p.dodgeTime - dt); p.invulnerable = Math.max(0, p.invulnerable - dt); p.consumableCooldown = Math.max(0, p.consumableCooldown - dt); p.disrupted = Math.max(0, p.disrupted - dt); if (p.ventT > 0) p.ventT = Math.max(0, p.ventT - dt);
...
function stepHazards(state: SimState, dt: number) {
  ...
  for (const hazard of state.hazards) {
    ...
    if (!p.dead && hazard.owner !== 'player' && playerDistance < hazard.radius) {
      if (hazard.kind === 'shockGrid') { p.disrupted = Math.max(p.disrupted, 0.45); if (Math.floor(state.time * 4) !== Math.floor((state.time - dt) * 4)) { if (hasTrait(state, 'stormskin')) { applyPlayerDamage(state, 2.2, 0.12); p.capacitor = Math.min(p.maxCapacitor, p.capacitor + 3); } else applyPlayerDamage(state, 4.2, 0.45); } }
