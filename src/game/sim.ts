...
export function triggerFire(state: SimState) {
  ...
  const projectileSpeed = weapon.projectileSpeed * (ghostline ? 1.35 : 1) * (lateralSpool ? 1.18 : 1) * (coldStartBreach ? 1.12 : 1) * pressureVelocity * (vectorSlipstream ? slipstreamSpeedMul : 1) * (redlineSlipstream ? 1.08 : 1);
  const shotDamage = weapon.damage * redlineScale * (nearBreach ? 1.22 : 1) * (pendulumBrake ? 1.22 : 1) * (cryoline ? 1.18 : 1) * (lateralSpool ? 1.22 : 1) * (coldStartBreach ? 1.24 : 1) * (vectorSlipstream && state.build.classResonanceTier >= 2 ? 1.08 : 1) * (redlineSlipstream ? 1.12 : 1);
  const shotPenetration = weapon.penetration + penetrationBonus + (ghostline ? 28 : 0) + (cryoline ? 24 : 0) + (coldStartBreach ? 18 : 0) + (pressurePsalm && sector.pressure < 0.45 ? 16 : 0) + (vectorSlipstream ? slipstreamPenetration : 0) + (redlineSlipstream ? 12 : 0);
...
export function triggerAbility(state: SimState, index = 0) {
  ...
  if (index === 0) {
    ...
    if (state.build.operatorClass === 'vanguard') {
      ...
      const shiftSpeed = (state.build.classResonanceTier >= 2 ? 760 : state.build.classResonanceTier >= 1 ? 700 : 640) * meta.power * (slingshotShift ? 1.18 : 1);
    ...
    if (state.build.operatorClass === 'vanguard' && state.build.mechanics.vanguardSiegeRam) {
      for (const enemy of state.enemies) {
        ...
        enemy.armor = Math.max(0, enemy.armor - 22 * meta.power);
    ...
    if (target && state.build.operatorClass === 'vector' && state.build.mechanics.vectorTriangulationLock) {
      target.statuses.armorBreach = Math.max(target.statuses.armorBreach, 2.8);
      p.abilityCooldowns[2] = Math.max(0, p.abilityCooldowns[2] - 1.2);
      spawnEffect(state, target.x, target.y, 'mark', 78, 0.6);
    }
    ...
    else if (target && state.build.operatorClass === 'vector') pushEvent(state, state.build.mechanics.vectorTriangulationLock ? `TRIANGULATION LOCK // ${target.label.toUpperCase()} // SPLITSHOT RECYCLED` : `DEADEYE LOCK // ${target.label.toUpperCase()} // SLIPSTREAM READY`, 1.35);
    ...
    if (state.build.mechanics.vanguardReprisalPulse) {
      for (const enemy of state.enemies) {
        if (!enemy.active || enemy.dead || enemy.statuses.armorBreach <= 0 || Math.hypot(enemy.x - p.x, enemy.y - p.y) > 330) continue;
        enemy.armor = Math.max(0, enemy.armor - 11 * meta.power);
    ...
    for (const angle of fanAngles) {
      ...
      addProjectile(state, p.x + dir.x * 30, p.y + dir.y * 30, dir, needleFan ? 1700 : 1480, 22 * meta.power * (centerLane ? 1.28 : 1), 'player', { weapon: 'rail', penetration: (state.build.classResonanceTier >= 2 ? 82 : 68) + (needleFan ? 20 : 0), armorDamage: needleFan ? 1.14 : 1.08, healthMultiplier: 0.96, knockback: 0.07, radius: 4 });
    }
    ...
    if (state.build.mechanics.arcCascadeLattice) { p.abilityCooldowns[0] = Math.max(0, p.abilityCooldowns[0] - 0.45); p.abilityCooldowns[1] = Math.max(0, p.abilityCooldowns[1] - 0.45); }
    spawnEffect(state, p.x + baseDir.x * 60, p.y + baseDir.y * 60, 'impact', needleFan ? 88 : 75, 0.35);
    pushEvent(state, needleFan ? 'NEEDLE FAN // HYPERVELOCITY CENTERLINE' : 'SPLITSHOT // THREE-LANE KINETIC FAN', 1.35);
  } else {
