...
export function triggerFire(state: SimState) {
  ...
  const projectileSpeed = weapon.projectileSpeed * (ghostline ? 1.35 : 1) * (lateralSpool ? 1.18 : 1) * (coldStartBreach ? 1.12 : 1) * pressureVelocity * (vectorSlipstream ? slipstreamSpeedMul : 1) * (redlineSlipstream ? 1.08 : 1) * (systemsCrossfeed ? 1.12 : 1);
  const shotDamage = weapon.damage * redlineScale * (nearBreach ? 1.22 : 1) * (pendulumBrake ? 1.22 : 1) * (cryoline ? 1.18 : 1) * (lateralSpool ? 1.22 : 1) * (coldStartBreach ? 1.24 : 1) * (vectorSlipstream && state.build.classResonanceTier >= 2 ? 1.08 : 1) * (redlineSlipstream ? 1.12 : 1) * (systemsCrossfeed ? 1.1 : 1);
  const shotPenetration = weapon.penetration + penetrationBonus + (ghostline ? 28 : 0) + (cryoline ? 24 : 0) + (coldStartBreach ? 18 : 0) + (pressurePsalm && sector.pressure < 0.45 ? 16 : 0) + (vectorSlipstream ? slipstreamPenetration : 0) + (redlineSlipstream ? 12 : 0) + (systemsCrossfeed ? 10 : 0);
...
export function triggerAbility(state: SimState, index = 0) {
  ...
  if (thermalShunterHeat >= 0.35) { const shuntedHeat = Math.min(thermalShunterHeat, state.build.specializationOverclock ? 0.1 : 0.08); p.weaponHeat[p.currentWeapon] = Math.max(0, p.weaponHeat[p.currentWeapon] - shuntedHeat); state.classState.systemsCrossfeed = state.build.specializationOverclock ? 3 : 2.6; pushEvent(state, `THERMAL SHUNTER // ${Math.round(shuntedHeat * 100)}% HEAT ROUTED // CROSSFIRE ARMED`, 1.3); }
