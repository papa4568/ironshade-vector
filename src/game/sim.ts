...
export type ClassRuntimeState = { vanguardGuard: number; vectorWindow: number; systemsLinks: number };
...
export function applyPlayerDamage(state: SimState, amount: number, armorPierce = 0) {
  ...
  const vanguardGuardScale = state.build.operatorClass === 'vanguard' && state.classState.vanguardGuard > 0 ? (state.build.classResonanceTier >= 2 ? 0.74 : 0.82) : 1;
...
export function createSimulation(build: CombatBuild = neutralCombatBuild): SimState {
  ...
  return {
    ...
    lastAbilityAt: -99,
    abilityChain: 0,
    classState: { vanguardGuard: 0, vectorWindow: 0, systemsLinks: 0 },
    bossGateHold: false,
    operationTier: 1,
    monsterLevel: 1,
    maxRecoveryLevel: 12,
...
export function triggerFire(state: SimState) {
  ...
  const projectileSpeed = weapon.projectileSpeed * (ghostline ? 1.35 : 1) * (lateralSpool ? 1.18 : 1) * (coldStartBreach ? 1.12 : 1) * pressureVelocity * (vectorSlipstream ? 1.22 : 1); const shotDamage = weapon.damage * redlineScale * (nearBreach ? 1.22 : 1) * (pendulumBrake ? 1.22 : 1) * (cryoline ? 1.18 : 1) * (lateralSpool ? 1.22 : 1) * (coldStartBreach ? 1.24 : 1) * (vectorSlipstream && state.build.classResonanceTier >= 2 ? 1.08 : 1); const shotPenetration = weapon.penetration + penetrationBonus + (ghostline ? 28 : 0) + (cryoline ? 24 : 0) + (coldStartBreach ? 18 : 0) + (pressurePsalm && sector.pressure < 0.45 ? 16 : 0) + (vectorSlipstream ? (state.build.classResonanceTier >= 2 ? 22 : 16) : 0); const shotKnockback = weapon.knockback * (nearBreach ? 1.75 : 1);
...
export function triggerDodge(state: SimState) {
  ...
  if (state.build.operatorClass === 'vector') { state.classState.vectorWindow = state.build.classResonanceTier >= 2 ? 1.75 : 1.25; pushEvent(state, 'VECTOR SLIPSTREAM // NEXT SHOT STABILIZED', 1.05); }
...
function stepPlayer(state: SimState, dt: number) {
  const p = state.player; const sector = currentSector(state, p.x, p.y); state.classState.vanguardGuard = Math.max(0, state.classState.vanguardGuard - dt); state.classState.vectorWindow = Math.max(0, state.classState.vectorWindow - dt); p.fireCooldown = Math.max(0, p.fireCooldown - dt); p.abilityCooldowns = p.abilityCooldowns.map(value => Math.max(0, value - dt)) as [number, number, number]; p.dodgeCooldown = Math.max(0, p.dodgeCooldown - dt); p.dodgeTime = Math.max(0, p.dodgeTime - dt); p.invulnerable = Math.max(0, p.invulnerable - dt); p.consumableCooldown = Math.max(0, p.consumableCooldown - dt); p.disrupted = Math.max(0, p.disrupted - dt); if (p.ventT > 0) p.ventT = Math.max(0, p.ventT - dt);
