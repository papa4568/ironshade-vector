...
export type SimState = { time: number; build: CombatBuild; weapons: Record<WeaponId, WeaponConfig>; droneTick: number; lastAbilityIndex: number; lastAbilityAt: number; abilityChain: number; bossGateHold: boolean; player: Player; enemies: Enemy[]; projectiles: Projectile[]; objects: CombatObject[]; sectors: Sector[]; links: PressureLink[]; breaches: Breach[]; hazards: Hazard[]; debris: Debris[]; effects: Effect[]; complete: boolean; bossActive: boolean; bossDefeated: boolean; pulse: number; weaponFlash: number; kills: number; squadSuppressing: boolean; eventText: string; eventT: number; telemetry: Telemetry };
...
export const weaponConfigs: Record<WeaponId, WeaponConfig> = {
  ...
  rail: { id: 'rail', name: 'Helix R-2 Rail Lance', shortName: 'R-2 RAIL LANCE', damage: 48, rate: 0.82, projectileSpeed: 1380, penetration: 115, recoil: 168, spread: 0.004, heatPerShot: 0.28, heatDissipation: 0.16, magazine: 5, reloadSeconds: 2.1, armorDamage: 1.75, healthMultiplier: 0.92, knockback: 0.12, pellets: 1, capacitorCost: 10 },
};
...
function damagePlayer(state: SimState, amount: number, armorPierce = 0) {
...
function dealEnemyDamage(state: SimState, enemy: Enemy, amount: number, armorDamageFactor: number, healthMultiplier: number, sourceKnockback = 0, sourceVelocity?: Vec2) {
...
export function aimAtMobileTarget(state: SimState, mode: 'light' | 'balanced' = 'balanced', preferredId: number | null = null) {
  ...
  if (preferred) {
    ...
    if (scored && (scored.visible || !bestVisible) && (!best || scored.score <= best.score + stickiness)) {
      p.aim = scored.direction;
      return preferred.id;
    }
  }

  if (!best) return null;
  p.aim = best.direction;
...
export function triggerFire(state: SimState) {
  ...
  const projectileSpeed = weapon.projectileSpeed * (ghostline ? 1.35 : 1) * (lateralSpool ? 1.18 : 1) * (coldStartBreach ? 1.12 : 1) * pressureVelocity; const shotDamage = weapon.damage * redlineScale * (nearBreach ? 1.22 : 1) * (pendulumBrake ? 1.22 : 1) * (cryoline ? 1.18 : 1) * (lateralSpool ? 1.22 : 1) * (coldStartBreach ? 1.24 : 1); const shotPenetration = weapon.penetration + penetrationBonus + (ghostline ? 28 : 0) + (cryoline ? 24 : 0) + (coldStartBreach ? 18 : 0) + (pressurePsalm && sector.pressure < 0.45 ? 16 : 0); const shotKnockback = weapon.knockback * (nearBreach ? 1.75 : 1);
  for (let pellet = 0; pellet < weapon.pellets; pellet += 1) { const spread = (rand() - 0.5) * weapon.spread * 2; const c = Math.cos(spread); const s = Math.sin(spread); const dir = norm({ x: p.aim.x * c - p.aim.y * s, y: p.aim.x * s + p.aim.y * c }); addProjectile(state, p.x + dir.x * 28, p.y + dir.y * 28, dir, projectileSpeed, shotDamage, 'player', { weapon: weapon.id, penetration: shotPenetration, armorDamage: weapon.armorDamage, healthMultiplier: weapon.healthMultiplier, knockback: shotKnockback, radius: weapon.id === 'rail' ? 5 : 4 }); }
  ...
  p.mags[p.currentWeapon] -= 1; p.capacitor = Math.max(0, p.capacitor - weapon.capacitorCost); p.fireCooldown = 1 / weapon.rate; p.weaponHeat[p.currentWeapon] = Math.min(1, p.weaponHeat[p.currentWeapon] + weapon.heatPerShot + (coldStartBreach ? 0.06 : 0) + (weapon.id === 'breacher' && hasTrait(state, 'redlineBulwark') && p.weaponHeat.breacher >= 0.72 ? 0.04 : 0)); if (nearBreach) p.weaponHeat[p.currentWeapon] = Math.max(0, p.weaponHeat[p.currentWeapon] - 0.06); if (hasTrait(state, 'cryostack') && p.weaponHeat[p.currentWeapon] >= 0.92) p.abilityCooldowns = p.abilityCooldowns.map(value => Math.max(0, value - 0.65)) as [number, number, number]; if (weapon.id === 'carbine' && hasTrait(state, 'palisadeDoctrine') && speed < 55) p.armor = Math.min(p.maxArmor, p.armor + 0.7); const gravity = sector.gravity; const recoilScale = (0.22 + (1 - gravity) * 0.92) * (ghostline ? 0.35 : 1) * (lateralSpool ? 0.55 : 1) * (weapon.id === 'rail' && hasTrait(state, 'recoilLedger') ? 1.3 : 1) * (weapon.id === 'carbine' && hasTrait(state, 'recoilDynamo') ? 1.18 : 1) * (weapon.id === 'breacher' && hasTrait(state, 'rheaBackblast') ? 1.45 : 1); const propulsion = weapon.id === 'breacher' && gravity < 0.15 && state.build.mechanics.breacherPropulsion ? 1 + 0.6 * (state.build.mechanics.breacherPropulsionScale || 1) : 1; const control = Math.max(0.68, 1 - state.build.player.lowGControl * (gravity < 0.35 ? 0.28 : 0.08)); const recoilForce = weapon.recoil * recoilScale * propulsion * control; if (state.build.mechanics.recoilVectoring && len(p.move) > 0.15) { p.vx += -p.aim.x * recoilForce * 0.65 + p.move.x * recoilForce * 0.35; p.vy += -p.aim.y * recoilForce * 0.65 + p.move.y * recoilForce * 0.35; } else { p.vx -= p.aim.x * recoilForce; p.vy -= p.aim.y * recoilForce; } const recoilReturn = Math.max(weapon.id === 'rail' && hasTrait(state, 'recoilLedger') ? 7 : 0, weapon.id === 'carbine' && hasTrait(state, 'recoilDynamo') ? Math.min(8, recoilForce * 0.16) : 0, state.build.specialization === 'momentum-broker' ? Math.min(state.build.specializationOverclock ? 10 : 8, recoilForce * 0.07) : 0, pendulumBrake ? 8 : 0); if (recoilReturn > 0) p.capacitor = Math.min(p.maxCapacitor, p.capacitor + recoilReturn); if (weapon.id === 'breacher' && hasTrait(state, 'rheaBackblast')) p.dodgeCooldown = Math.max(0, p.dodgeCooldown - 0.22); if (pendulumBrake) { p.vx *= 0.55; p.vy *= 0.55; pushEvent(state, 'PENDULUM KESTREL // COUNTER-IMPULSE BANKED', 1.2); } state.weaponFlash = 0.07; state.telemetry.weaponShots[p.currentWeapon] += 1; if (p.mags[p.currentWeapon] === 0) triggerReload(state); return true;
}
export function triggerDodge(state: SimState) {
  ...
  p.vx = dir.x * speed; p.vy = dir.y * speed; p.dodgeTime = 0.18; p.invulnerable = 0.24; p.dodgeCooldown = scrapline ? 1.0 : 1.3;
...
export function triggerAbility(state: SimState, index = 0) {
  ...
  if (index === 0) {
    state.pulse = 0.36;
    for (const enemy of state.enemies) { if (enemy.dead || !enemy.active) continue; const delta = { x: enemy.x - p.x, y: enemy.y - p.y }; const distance = len(delta); if (distance > 285 || distance < 1) continue; const dir = norm(delta); if (dir.x * p.aim.x + dir.y * p.aim.y < 0.1) continue; const exosuit = enemy.variant === 'meleeExosuit'; const anchored = !exosuit && (enemy.combatClass === 'elite' || enemy.role === 'boss' || protocolAnchorsEnemy(enemy) || isAnchoredByElite(state, enemy)) && enemy.statuses.disrupted <= 0; const force = 610 * meta.power * (1 - distance / 390) * (anchored ? 0.18 : 1) * (exosuit ? 1.75 : 1); enemy.vx += dir.x * force; enemy.vy += dir.y * force; enemy.statuses.stagger = Math.max(enemy.statuses.stagger, staggerDuration(enemy, exosuit ? 1.35 : anchored ? 0.2 : 0.8)); dealEnemyDamage(state, enemy, (exosuit ? 13 : 8) * meta.power, 0.28, 0.6); }
    ...
    const counterKick = state.build.mechanics.magOverdriveKick ? 104 : 62; p.vx -= p.aim.x * counterKick; p.vy -= p.aim.y * counterKick;
    ...
    const sector = currentSector(state, p.x, p.y); if (state.build.specialization === 'pressure-diver' && sector.pressure < 0.45) plantHazard(state, p.x + p.aim.x * 145, p.y + p.aim.y * 145, 'vacuumWake', state.build.specializationOverclock ? 3 : 2.2, 'player');
  ...
  if (chained) { state.abilityChain = Math.min(2, state.abilityChain + 1); if (state.build.specialization === 'capacitor-conductor') { const desiredRefund = state.build.specializationOverclock && state.abilityChain >= 2 ? 8 : 6; const existingRecovery = Math.max(0, p.capacitor - capacitorAfterCost); const refund = Math.max(0, Math.min(desiredRefund, 8 - Math.min(8, existingRecovery))); if (refund > 0) p.capacitor = Math.min(p.maxCapacitor, p.capacitor + refund); if (state.build.specializationOverclock && state.abilityChain >= 2) p.weaponHeat[p.currentWeapon] = Math.max(0, p.weaponHeat[p.currentWeapon] - 0.08); } if (hasTrait(state, 'abilityRosary')) { p.weaponHeat[p.currentWeapon] = Math.max(0, p.weaponHeat[p.currentWeapon] - 0.06); if (previousAbility >= 0) p.abilityCooldowns[previousAbility] = Math.max(0, p.abilityCooldowns[previousAbility] - 0.3); } } else state.abilityChain = 0;
...
export function getContextAction(state: SimState) {
  ...
  for (const object of state.objects) {
    ...
    if ((object.id === 'gravity-control-a' || object.id === 'gravity-control-b' || object.id === 'boarding-lock' || object.id === 'solar-shutter' || object.id === 'capture-drum-a' || object.id === 'capture-drum-b' || object.id === 'purge-valve-a' || object.id === 'purge-valve-b') && object.exposed) continue;
...
function stepPlayer(state: SimState, dt: number) {
  const p = state.player; const sector = currentSector(state, p.x, p.y); p.fireCooldown = Math.max(0, p.fireCooldown - dt); p.abilityCooldowns = p.abilityCooldowns.map(value => Math.max(0, value - dt)) as [number, number, number]; p.dodgeCooldown = Math.max(0, p.dodgeCooldown - dt); p.dodgeTime = Math.max(0, p.dodgeTime - dt); p.invulnerable = Math.max(0, p.invulnerable - dt); p.disrupted = Math.max(0, p.disrupted - dt); if (p.ventT > 0) p.ventT = Math.max(0, p.ventT - dt);
  ...
  if (sector.pressure < 0.18) { p.vacuumExposure += dt * (1 - state.build.player.vacuumResistance * 0.55); if (p.vacuumExposure > 1.8 + state.build.player.vacuumResistance * 2.4) damagePlayer(state, 3.2 * (1 - state.build.player.vacuumResistance * 0.78) * dt, 1); } else p.vacuumExposure = Math.max(0, p.vacuumExposure - dt * 1.8);
  applyPressureForce(state, p, sector.id, dt, 0.48 * (1 - state.build.player.vacuumResistance * 0.45));
  if (!p.dead && p.dodgeTime <= 0) { const glasswalker = hasTrait(state, 'glasswalker') && sector.pressure < 0.18; const redlineMove = state.build.specialization === 'redline-pilot' && p.weaponHeat[p.currentWeapon] >= 0.75; const accel = lerp(880, 1210, sector.gravity) * (glasswalker ? 1.22 : 1) * (redlineMove ? 1.05 : 1); const maxSpeed = 278 * state.build.player.moveSpeedMul * (glasswalker ? 1.18 : 1) * (redlineMove ? 1.08 : 1); const mobilityScale = p.ventT > 0 ? 0.72 : p.disrupted > 0 ? 0.82 : 1; p.vx += p.move.x * accel * mobilityScale * state.build.player.moveSpeedMul * dt; p.vy += p.move.y * accel * mobilityScale * state.build.player.moveSpeedMul * dt; const speed = Math.hypot(p.vx, p.vy); if (speed > maxSpeed) { p.vx *= maxSpeed / speed; p.vy *= maxSpeed / speed; } const damping = Math.pow(Math.max(0.0009, lerp(0.48, 0.0009, sector.gravity) - (sector.gravity < 0.35 ? state.build.player.lowGControl * 0.16 : 0)), dt); p.vx *= damping; p.vy *= damping; } else if (p.dodgeTime <= 0) { p.vx *= Math.pow(0.06, dt); p.vy *= Math.pow(0.06, dt); }
...
function stepEnemy(state: SimState, enemy: Enemy, dt: number) {
  ...
  const p = state.player; if (p.dead) return; enemy.fireCooldown = Math.max(0, enemy.fireCooldown - dt); enemy.hazardCooldown = Math.max(0, enemy.hazardCooldown - dt); const sector = currentSector(state, enemy.x, enemy.y); const anchorProtected = enemy.combatClass === 'elite' || protocolAnchorsEnemy(enemy) || (isAnchoredByElite(state, enemy) && enemy.statuses.disrupted <= 0); applyPressureForce(state, enemy, sector.id, dt, anchorProtected ? 0.12 : 0.92);
  ...
  const delta = { x: p.x - enemy.x, y: p.y - enemy.y }; const distance = len(delta); const toward = norm(delta); const sideways = { x: -toward.y * enemy.strafeSign, y: toward.x * enemy.strafeSign }; const retreat = pressureRetreatVector(state, enemy); const hasPressureThreat = len(retreat) > 0.1; const recoilMover = enemy.variant === 'vectorSkirmisher' || enemy.variant === 'tetherRigger' || enemy.variant === 'impulseRigger' || enemy.variant === 'recoilBroker'; const droneMover = enemy.variant === 'maintenanceDrone' || enemy.variant === 'gravityDrone' || enemy.variant === 'repairDrone'; let desired = { x: 0, y: 0 }; let desiredDistance = enemy.role === 'assault' ? (state.squadSuppressing ? 145 : 205) : enemy.role === 'suppressor' ? 470 : enemy.role === 'technician' ? 410 : 330; if (recoilMover) desiredDistance = 305; else if (enemy.variant === 'anchorEngineer') desiredDistance = 390; else if (droneMover) desiredDistance = enemy.variant === 'gravityDrone' ? 430 : 340; else if (enemy.variant === 'marksman') desiredDistance = 690; else if (enemy.variant === 'meleeExosuit') desiredDistance = 72; else if (enemy.variant === 'shieldBoarder') desiredDistance = 175; else if (enemy.variant === 'salvageThief' || enemy.variant === 'custodyPorter') desiredDistance = enemy.carriedObjectId ? 900 : 260;
  ...
  if (enemy.telegraph > 0) { enemy.telegraph -= dt; if (enemy.telegraph <= 0) { if (enemy.variant === 'meleeExosuit') { if (distance < 145) { const shove = norm({ x: p.x - enemy.x, y: p.y - enemy.y }); damagePlayer(state, 24, 0.08); p.vx += shove.x * 360; p.vy += shove.y * 360; } } else if (enemy.variant === 'marksman') fireEnemyShot(state, enemy, 980, 25); else if (enemy.role === 'assault') fireEnemyShot(state, enemy, 500, 10, 0.115); else if (enemy.role === 'elite') fireEnemyShot(state, enemy, 560, 22); else fireEnemyShot(state, enemy, 465, 17); if (recoilMover) { enemy.vx -= enemy.telegraphAim.x * 155; enemy.vy -= enemy.telegraphAim.y * 155; } enemy.fireCooldown = enemy.variant === 'marksman' ? 2.4 : enemy.variant === 'meleeExosuit' ? 1.65 : enemy.role === 'assault' ? 1.35 : enemy.role === 'suppressor' ? 1.1 : enemy.role === 'elite' ? 1.25 : 1.7; if (enemy.role === 'suppressor' && enemy.variant !== 'marksman') enemy.burst = 2; } }
  ...
  else { const attackRange = enemy.variant === 'marksman' ? 930 : enemy.variant === 'meleeExosuit' ? 155 : enemy.role === 'assault' ? 330 : 690; if (enemy.fireCooldown <= 0 && distance < attackRange && clearLine(state, enemy.x, enemy.y, p.x, p.y)) { enemy.telegraph = enemy.variant === 'marksman' ? 1.45 : enemy.variant === 'meleeExosuit' ? 0.62 : enemy.role === 'assault' ? 0.45 : enemy.role === 'elite' ? 0.64 : 0.72; enemy.telegraphAim = toward; enemy.state = 'attack'; } }
}
...
function stepPressureBrokerBoss(state: SimState, boss: Enemy, dt: number) {
  ...
  if (boss.telegraph > 0) {
    ...
    else if (boss.bossPattern === 'latticePulse') { const delta = { x: boss.x - p.x, y: boss.y - p.y }; const distance = len(delta); if (distance < 390) { const dir = norm(delta); p.vx += dir.x * 430; p.vy += dir.y * 430; p.disrupted = Math.max(p.disrupted, 0.7); damagePlayer(state, boss.bossPhase === 2 ? 19 : 15, 0.18); } spawnEffect(state, boss.x, boss.y, 'pulse', 360, 0.7); }
...
function stepHazards(state: SimState, dt: number) {
  ...
  for (const hazard of state.hazards) {
    ...
    if (hazard.kind === 'vacuumWake') { for (const enemy of state.enemies) { if (!enemy.active || enemy.dead) continue; const d = Math.hypot(enemy.x - hazard.x, enemy.y - hazard.y); if (d <= 0 || d >= hazard.radius) continue; const dir = norm({ x: hazard.x - enemy.x, y: hazard.y - enemy.y }); const pull = 300 * (1 - d / hazard.radius); enemy.vx += dir.x * pull * dt; enemy.vy += dir.y * pull * dt; enemy.statuses.vacuum = Math.max(enemy.statuses.vacuum, 0.8); dealEnemyDamage(state, enemy, 5 * dt, 0.18, 0.75); } continue; }
    const playerDistance = Math.hypot(p.x - hazard.x, p.y - hazard.y);
    if (!p.dead && hazard.owner !== 'player' && playerDistance < hazard.radius) {
      if (hazard.kind === 'shockGrid') { p.disrupted = Math.max(p.disrupted, 0.45); if (Math.floor(state.time * 4) !== Math.floor((state.time - dt) * 4)) { if (hasTrait(state, 'stormskin')) { damagePlayer(state, 2.2, 0.12); p.capacitor = Math.min(p.maxCapacitor, p.capacitor + 3); } else damagePlayer(state, 4.2, 0.45); } }
