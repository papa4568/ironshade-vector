...
export function triggerAbility(state: SimState, index = 0) {
  ...
  if (index === 0) {
    ...
    for (const enemy of state.enemies) { if (enemy.dead || !enemy.active) continue; const delta = { x: enemy.x - p.x, y: enemy.y - p.y }; const distance = len(delta); if (distance > 285 || distance < 1) continue; const dir = norm(delta); if (dir.x * p.aim.x + dir.y * p.aim.y < 0.1) continue; const exosuit = enemy.variant === 'meleeExosuit'; const anchored = !exosuit && (enemy.combatClass === 'elite' || enemy.role === 'boss' || protocolAnchorsEnemy(enemy) || isAnchoredByElite(state, enemy)) && enemy.statuses.disrupted <= 0; const polarityPoint = { x: p.x + p.aim.x * 185, y: p.y + p.aim.y * 185 }; const impulseDir = state.build.operatorClass === 'systems' ? norm({ x: polarityPoint.x - enemy.x, y: polarityPoint.y - enemy.y }) : dir; const classForce = state.build.operatorClass === 'vanguard' ? 1.3 : state.build.operatorClass === 'vector' ? 0.55 : state.build.operatorClass === 'systems' ? 1.18 : 1; const force = 610 * meta.power * classForce * (1 - distance / 390) * (anchored ? 0.18 : 1) * (exosuit ? 1.75 : 1); enemy.vx += impulseDir.x * force; enemy.vy += impulseDir.y * force; enemy.statuses.stagger = Math.max(enemy.statuses.stagger, staggerDuration(enemy, state.build.operatorClass === 'vanguard' ? 1.05 : exosuit ? 1.35 : anchored ? 0.2 : 0.8)); if (state.build.operatorClass === 'systems') enemy.statuses.disrupted = Math.max(enemy.statuses.disrupted, 1.6); dealEnemyDamage(state, enemy, (state.build.operatorClass === 'vanguard' ? 13 : exosuit ? 13 : 8) * meta.power, state.build.operatorClass === 'vanguard' ? 0.65 : 0.28, 0.6); }
    ...
    const slotOneEvent = state.build.operatorClass === 'vanguard'
      ? 'BREACH RUSH // RAM LINE COMMITTED // GUARD UP'
      : state.build.operatorClass === 'vector'
        ? 'VECTOR SHIFT // SLIPSTREAM PRIMED'
        : state.build.operatorClass === 'systems'
          ? 'POLARITY WELL // FORMATION COLLAPSED'
    ...
    if (target && state.build.operatorClass === 'vanguard') pushEvent(state, `FRACTURE TAG // ${target.label.toUpperCase()} ARMOR PATH OPEN`, 1.35);
    ...
    for (const enemy of state.enemies) {
      ...
      const delta = { x: enemy.x - p.x, y: enemy.y - p.y };
      const distance = len(delta);
      if (distance <= 0 || distance > 330) continue;
      const away = norm(delta);
      enemy.vx += away.x * 360 * (1 - distance / 390);
      enemy.vy += away.y * 360 * (1 - distance / 390);
      enemy.statuses.stagger = Math.max(enemy.statuses.stagger, staggerDuration(enemy, 0.9));
      dealEnemyDamage(state, enemy, 16 * meta.power, 0.85, 0.72);
      hits += 1;
    }
