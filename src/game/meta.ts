...
function gradedDescription(id: AffixId, grade: ModifierGrade) {
  ...
  if (id === 'magRedirect') return `The first class skill redirects hostile projectiles; redirected kinetic payload scales to ${gradePercent(100 * power)}% of the standard return.`;
...
const chaseCatalog: SingularTemplate[] = [
  ...
  singular({ baseId: 'scrap-circuit-rig', name: 'Scrap Circuit Rig', slot: 'rig', equipmentClass: 'Destruction-to-Arc recovery bus', rarity: 'Singular', core: 'An ice-mine field bus that uses the electrical collapse of machinery to precharge Cascade Arc instead of harvesting the wreck for raw damage.', modifiers: [{ ...affixes.arcDrone }, { ...affixes.capacitorRecycler }, { ...affixes.dodgeVent }], singularTrait: 'scrapCircuit', singularEffect: 'Destroying machinery spends 4 capacitor to advance your third class skill recovery by 0.9 seconds.' }),
  ...
  singular({ baseId: 'bloom-vector-rig', name: 'Bloom Vector Rig', slot: 'rig', equipmentClass: 'Radial impulse recovery rig', rarity: 'Singular', core: 'A ring of sacrificial micro-coils turns the first class-skill field collapse into a brief omnidirectional kinetic bloom.', modifiers: [{ ...affixes.capacitorRecycler }, { ...affixes.magRedirect }, { ...affixes.servoWeave }], singularTrait: 'magBloom', singularEffect: 'Your first class skill fires eight radial kinetic micro-slugs after resolving. That skill costs 25% more capacitor and recovers 8% slower.' }),
  singular({ baseId: 'cascade-sight-link', name: 'Cascade Sight Link', slot: 'implant', equipmentClass: 'Kill-relay sensor cognition link', rarity: 'Singular', core: 'A narrowband target model refuses to hold one solution for long, but transfers the dying target state into the nearest live return.', modifiers: [{ ...affixes.markShear }, { ...affixes.arcDrone }, { ...affixes.capacitorRecycler }], singularTrait: 'markCascade', singularEffect: 'Killing a target marked by your second class skill relays a 4.2s mark to a nearby enemy. Initial second-skill marks are shorter and the skill recovers 12% slower.' }),
];
...
export const specializationDefinitions: SpecializationDefinition[] = [
  ...
  { id: 'grid-weaver', operatorClass: 'systems', name: 'Grid Weaver', identity: 'Machinery-network Arc routing', description: 'Cascade Arc through machinery can paint an additional remote target for Relay Hack follow-up.', tradeoff: '-4% direct weapon output.', overclock: 'Machinery Cascade Arc also advances Relay Hack recovery.', overclockTradeoff: '+15% Cascade Arc capacitor cost.' },
  { id: 'survey-deadeye', operatorClass: 'vector', name: 'Survey Deadeye', identity: 'Marked-target rail precision', description: 'Rail hits consume marks to break committed attacks, create a short Armor Breach window, and re-prime Slipstream for a precision follow-through.', tradeoff: 'Deadeye Lock marks are 20% shorter and recover 10% slower.', overclock: 'Precision traces pull Deadeye Lock toward a 1.6 second recovery window and extend the follow-through Slipstream window.', overclockTradeoff: '+8% Rail Lance heat per shot.' },
  ...
  { id: 'capacitor-conductor', operatorClass: 'systems', name: 'Capacitor Conductor', identity: 'Ability-cycle combo routing', description: 'Casting a different Systems class skill within 3.4 seconds returns capped capacitor and rewards deliberate three-button sequencing.', tradeoff: '-12 maximum capacitor.', overclock: 'Completing the third link of a sequence raises the capped refund and cools the active weapon.', overclockTradeoff: '+10% ability capacitor cost.' },
...
export const operatorClassDefinitions: OperatorClassDefinition[] = [
  ...
  },
  {
    ...
    signatureDescription: 'Chaining different Systems class skills advances the previous ability. Completing the three-link loop recycles capacitor and sheds weapon heat.',
...
function applyOperatorNetworkStatEffect(build: CombatBuild, effect: OperatorNetworkStatEffect) {
  ...
  if (effect.stat === 'attack-speed-mul') build.attackSpeedMul *= effect.value;
