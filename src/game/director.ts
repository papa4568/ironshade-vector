...
function configureTacticalRoster(state: SimState, contract: Contract) {
  ...
  else if (contract.location === 'parallax-array') { setTacticalEnemy(state, 1, 'assault', 'parallaxSkirmisher', 'Parallax Shear Runner', 100, 50); setTacticalEnemy(state, 2, 'suppressor', 'baselineMarksman', 'Long-Baseline Marksman', 82, 38); setTacticalEnemy(state, 3, 'technician', 'referenceTech', 'Reference Shear Technician', 88, 44); setTacticalEnemy(state, 4, 'assault', 'parallaxSkirmisher', 'Cross-Track Runner', 102, 52); setTacticalEnemy(state, 5, 'technician', 'repairDrone', 'Array Calibration Drone', 68, 30); setTacticalEnemy(state, 6, 'elite', 'referenceTech', 'Senior Baseline Controller', 182, 132); setTacticalEnemy(state, 7, 'assault', 'parallaxSkirmisher', 'Reserve Shear Runner', 94, 44); setTacticalEnemy(state, 8, 'suppressor', 'baselineMarksman', 'Deep-Reference Marksman', 80, 36); }
}
...
export function applyMissionSetup(state: SimState, contract: Contract) {
  ...
  } else if (contract.location === 'parallax-array') {
    state.sectors[0].label = 'NEAR BASELINE'; state.sectors[1].label = 'CROSS-TRACK GALLERY'; state.sectors[2].label = 'DEEP REFERENCE'; state.sectors[0].gravity = 0.48; state.sectors[1].gravity = 0.08; state.sectors[2].gravity = 0.22; state.sectors[0].pressure = 0.9; state.sectors[1].pressure = 0.82; state.sectors[2].pressure = 0.86;
  } else {
...
export function stepMissionDirector(state: SimState, runtime: DirectorRuntime, contract: Contract, dt: number) {
  ...
  if (contract.location === 'spin-habitat') {
    ...
    if (!runtime.locationEventA && runtime.elapsed >= 8) { runtime.locationEventA = true; if (objective.complete) event(state, 'REFERENCE CONVERGENCE // FIRST SHEAR FRONT CANCELLED', 3); else { state.sectors[0].gravity = 0.08; state.sectors[1].gravity = 0.52; deployHazard(state, 980, 420, 'vectorWash', 6); event(state, 'PARALLAX SHEAR // NEAR AND CROSS-TRACK REFERENCES DISAGREE // VECTOR FRONT LIVE', 3.6); } }
    if (!runtime.locationEventB && runtime.elapsed >= 19) { runtime.locationEventB = true; if (objective.complete) { state.sectors[0].gravity = 0.48; state.sectors[1].gravity = 0.18; state.sectors[2].gravity = 0.22; event(state, 'LONG-BASELINE SOLUTION LOCKED // SECOND SHEAR DAMPED', 3); } else { state.sectors[1].gravity = 0.04; state.sectors[2].gravity = 0.56; deployHazard(state, 1420, 620, 'vectorWash', 6); event(state, 'DEEP REFERENCE REVERSAL // SECOND SHEAR FRONT CROSSING ARRAY', 3.5); } }
  } else if (contract.location === 'lattice-annex') {
    if (!runtime.locationEventA && runtime.elapsed >= 10) {
      runtime.locationEventA = true;
