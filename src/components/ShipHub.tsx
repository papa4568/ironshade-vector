...
import { parallaxDebtChapter, parallaxDebtEvidence, parallaxDebtNextRequiredLevel } from '../game/parallaxDebt';
...
export default function ShipHub({ profile, campaign, contracts, operations, operationsStatus, operationsError, telemetrySharing, selectedContractId, statusMessage, onSelectContract, onDeploy, onOpenBuild, onCampaignChange }: Props) {
  ...
  return <main className={`ship-hub area-${primaryArea}`}>
    ...
    {tab === 'campaign' && <section className="campaign-panel">
      ...
      <article className="campaign-hero"><header><div><span className="card-kicker">P1 // PARALLAX DEBT // LV15–18</span><h2>{parallaxDebtChapter.title}</h2><p>Reconstruct the private cislunar reference as a physical route instead of a theory. The opening three operations now feed six additional missions across LV16–18, with progression gates that deliberately send under-level operators back to side contracts rather than flattening the campaign curve.</p></div><span className="campaign-state">{parallaxProgress.status.toUpperCase()}</span></header><div className="campaign-meter"><div><i style={{ width: `${Math.min(100, parallaxProgress.step / parallaxDebtChapter.totalContracts * 100)}%` }} /></div><b>{Math.min(parallaxProgress.step, parallaxDebtChapter.totalContracts)} / {parallaxDebtChapter.totalContracts} CONTRACTS · {parallaxDebtChapter.authoredContracts} AUTHORED</b></div><p>{parallaxProgress.lastBeat}</p><div className="campaign-actions">{parallaxProgress.status === 'locked' && <span>{campaign.story.interdiction.status !== 'complete' ? 'Complete Interdiction before opening Parallax Debt.' : 'Reach operator level 15 to calibrate the Cislunar Parallax Array.'}</span>}{parallaxProgress.status === 'active' && parallaxContract && <button className="primary" onClick={openParallaxContract}>Open contract {parallaxProgress.step + 1}</button>}{parallaxProgress.status === 'active' && !parallaxContract && parallaxRequiredLevel !== null && profile.level < parallaxRequiredLevel && <span>NEXT PHASE LOCK // reach LV{parallaxRequiredLevel}. Standard, Directive, Command Trace, and other available contracts continue advancing operator XP.</span>}{parallaxProgress.status === 'active' && !parallaxContract && parallaxRequiredLevel === null && <span>MID-CHAPTER HOLD // nine LV15–18 contracts are authored. The campaign remains open for its decision branch and final three operations.</span>}{parallaxProgress.status === 'complete' && <span>Parallax Debt complete // the private reference route is physically resolved through its final branch.</span>}</div>{parallaxFindings.length > 0 && <div className="campaign-evidence-list">{parallaxFindings.map((entry, index) => <span key={entry}>{String(index + 1).padStart(2, '0')} // {entry}</span>)}</div>}</article>
