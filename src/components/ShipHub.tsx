...
import { environmentalRiskForecastForContract } from '../game/environmentalRiskPackages';
...
import { bossPhaseMutationForecastForContract } from '../game/bossPhaseMutations';
import { bossPhaseMutationPresentationFor } from '../game/bossPhaseMutationPresentation';
import { commandTargetMutationForecastForContract, commandTargetMutationName } from '../game/commandTargetMutations';
...
export default function ShipHub({ profile, campaign, contracts, operations, operationsStatus, operationsError, telemetrySharing, selectedContractId, statusMessage, onSelectContract, onDeploy, onOpenBuild, onCampaignChange }: Props) {
  ...
  const commandTargetMutationForecast = selected ? commandTargetMutationForecastForContract(selected) : [];
  const bossPhaseMutationForecast = selected ? bossPhaseMutationForecastForContract(selected) : [];
  ...
  const focusPrimaryAreaByOffset = (offset: number, edge?: 'start' | 'end') => {
    const buttons = [...(commandNavRef.current?.querySelectorAll<HTMLButtonElement>('button[data-primary-area]') ?? [])];
    if (!buttons.length) return;
    const activeIndex = buttons.indexOf(document.activeElement as HTMLButtonElement);
    const selectedIndex = Math.max(0, buttons.findIndex(button => button.dataset.primaryArea === primaryArea));
  ...
  const handlePrimaryNavigation = (event: React.KeyboardEvent<HTMLElement>) => {
    if (event.key === 'ArrowRight' || event.key === 'ArrowDown') { event.preventDefault(); focusPrimaryAreaByOffset(1); }
    if (event.key === 'ArrowLeft' || event.key === 'ArrowUp') { event.preventDefault(); focusPrimaryAreaByOffset(-1); }
    if (event.key === 'Home') { event.preventDefault(); focusPrimaryAreaByOffset(0, 'start'); }
    if (event.key === 'End') { event.preventDefault(); focusPrimaryAreaByOffset(0, 'end'); }
  };
  ...
  useEffect(() => {
    ...
    const poll = () => {
      ...
      if (direction && direction !== previousDirection) {
        ...
        if (active instanceof HTMLButtonElement && commandNavRef.current?.contains(active)) focusPrimaryAreaByOffset(direction);
        else commandNavRef.current?.querySelector<HTMLButtonElement>(`button[data-primary-area="${primaryArea}"]`)?.focus();
      }
      ...
      if (confirm && !previousConfirm) {
        const active = document.activeElement;
        if (active instanceof HTMLButtonElement && commandNavRef.current?.contains(active)) active.click();
        else commandNavRef.current?.querySelector<HTMLButtonElement>(`button[data-primary-area="${primaryArea}"]`)?.focus();
      }
  ...
  return <main className={`ship-hub area-${primaryArea} iv-view`}>
    <aside className="command-rail" aria-label="Primary navigation">
      ...
      <nav ref={commandNavRef} className="command-rail-nav" onKeyDown={handlePrimaryNavigation}>
    ...
    {tab === 'ship' && <section className="ship-systems-panel">
      ...
      <section className="upgrade-group ship-specialization-group">
        ...
        <div className="upgrade-grid specialization-grid">
          {shipSpecializationDefinitions.map(definition => {
            ...
            return <article key={definition.id} className={`upgrade-card iv-panel specialization-card${status.selected ? ' specialization-selected' : ''}`}>
      ...
      {upgradeGroups.map(([area, upgrades]) => <section key={area} className="upgrade-group"><h2>{area}</h2><div className="upgrade-grid">{upgrades.map(upgrade => { const status = getShipUpgradeStatus(campaign, upgrade.id)!; const level = status.level; const cost = getUpgradeCost(campaign, upgrade.id); const nextTier = status.nextTier; const currentBenefit = level > 0 ? upgrade.benefits[level - 1] : 'No major tier installed yet.'; const buttonLabel = cost ? `Upgrade // ${costText(cost)}` : !nextTier ? 'System fully realized' : status.implementationLocked ? `Tier ${nextTier.tier} blueprint queued` : `Tier ${nextTier.tier} gated`; return <article key={upgrade.id} className="upgrade-card iv-panel"><div><small>{upgrade.name.toUpperCase()}</small><b>TIER {level} / {SHIP_SYSTEM_MAX_TIER}</b></div><p>{upgrade.description}</p><strong>{currentBenefit}</strong>{nextTier && <div className="upgrade-plan"><small>NEXT // TIER {nextTier.tier}</small><span>{nextTier.benefit}</span>{nextTier.gates.length > 0 && <em>{status.gateFailures.length > 0 ? `Gates: ${status.gateFailures.map(gate => gate.label).join(' · ')}` : 'All access gates satisfied'}</em>}</div>}<button disabled={!cost} onClick={() => buy(upgrade.id)}>{buttonLabel}</button></article>; })}</div></section>)}
    </section>}
    ...
    {tab === 'cargo' && <section className="cargo-panel"><article className="consumable-store"><span className="card-kicker">FIELD CONSUMABLES // SHIP STORE</span><h2>Spend Credits on deployment supplies</h2><p>Credits are the field currency. Supplies persist in Quiet Signal storage and are only consumed when an effect successfully activates in combat.</p><div className="consumable-credit-balance"><small>AVAILABLE CREDITS</small><b>{campaign.resources.credits}</b></div><div className="consumable-shop-grid">{consumableDefinitions.map(item => { const stock = campaign.consumables[item.id]; const full = stock >= item.maxStock; const affordable = campaign.resources.credits >= item.cost; return <div key={item.id} className="consumable-shop-card"><header><div><small>{item.hotkey} // {item.shortName}</small><b>{item.name}</b></div><strong>{stock}/{item.maxStock}</strong></header><p>{item.description}</p><span>{item.effect}</span><button disabled={full || !affordable} onClick={() => buySupply(item.id)}>{full ? 'Stock full' : `Buy // ${item.cost} Credits`}</button></div>; })}</div></article><article className="cargo-ledger"><span className="card-kicker">CARGO / SALVAGE LEDGER</span><h2>Banked resources</h2>{(Object.keys(campaign.resources) as ResourceId[]).map(key => <div key={key}><span>{resourceLabels[key]}</span><b>{campaign.resources[key]}</b></div>)}</article><article className="cargo-ledger"><span className="card-kicker">EQUIPMENT STORAGE</span><h2>{profile.inventory.length} equipment packages</h2><p>Equipment recovery remains sparse and meaningful. Cargo upgrades increase material yield, not item spam.</p><button onClick={onOpenBuild}>Inspect equipment</button></article><article className="cargo-ledger"><span className="card-kicker">LAST OPERATION</span><h2>Mission log</h2><p>{campaign.lastOutcome}</p>{campaign.resources.rareTech > 0 && <div className="anomaly-note"><b>QUARANTINED TRACE // {campaign.resources.rareTech}</b><span>Recovered lattice material occupies shielded sample storage. Its geometry has not been matched to normal human industrial methods; the Campaign dossier tracks what Quiet Signal can actually support from the evidence.</span></div>}</article></section>}
      {commissionCeremony && <div className="ship-commission-overlay" role="dialog" aria-modal="true" aria-label="Ship system commissioned" onClick={() => setCommissionCeremony(null)}>
        <article className={`ship-commission-card iv-panel mechanism-${commissionCeremony.mechanism}`} onClick={event => event.stopPropagation()}>
