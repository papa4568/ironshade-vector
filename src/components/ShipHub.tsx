...
import { chooseParallaxDebtBranch, getParallaxDebtChoicePrompt, parallaxDebtChapter, parallaxDebtEvidence, parallaxDebtNextRequiredLevel } from '../game/parallaxDebt';
...
export default function ShipHub({ profile, campaign, contracts, operations, operationsStatus, operationsError, telemetrySharing, selectedContractId, statusMessage, onSelectContract, onDeploy, onOpenBuild, onCampaignChange }: Props) {
  ...
  return <main className={`ship-hub area-${primaryArea}`}>
    ...
    {tab === 'campaign' && <section className="campaign-panel">
      ...
      <article className="campaign-hero"><header><div><span className="card-kicker">P1 // PARALLAX DEBT // LV15–18</span><h2>{parallaxDebtChapter.title}</h2><p>Reconstruct the private cislunar reference as a physical route instead of a theory. Nine operations establish the LV15–18 route, then False Horizon forces a campaign decision that changes all three authored closing operations and the final strategic outcome.</p></div><span className="campaign-state">{parallaxProgress.status.toUpperCase()}</span></header><div className="campaign-meter"><div><i style={{ width: `${Math.min(100, parallaxProgress.step / parallaxDebtChapter.totalContracts * 100)}%` }} /></div><b>{Math.min(parallaxProgress.step, parallaxDebtChapter.totalContracts)} / {parallaxDebtChapter.totalContracts} CONTRACTS · {parallaxDebtChapter.authoredContracts} AUTHORED</b></div><p>{parallaxProgress.lastBeat}</p>{parallaxChoice && <div className="campaign-choice"><b>{parallaxChoice.title}</b><span>{parallaxChoice.body}</span><div className="campaign-choice-options">{parallaxChoice.choices.map(choice => <button key={choice.id} onClick={() => { const next = chooseParallaxDebtBranch(campaign, choice.id); onCampaignChange(next); setMessage(next.story.parallaxDebt.lastBeat); }}><b>{choice.title}</b><span>{choice.body}</span><small>{choice.consequence}</small></button>)}</div></div>}<div className="campaign-actions">{parallaxProgress.status === 'locked' && <span>{campaign.story.interdiction.status !== 'complete' ? 'Complete Interdiction before opening Parallax Debt.' : 'Reach operator level 15 to calibrate the Cislunar Parallax Array.'}</span>}{parallaxProgress.status === 'active' && parallaxContract && !parallaxChoice && <button className="primary" onClick={openParallaxContract}>Open contract {parallaxProgress.step + 1}</button>}{parallaxProgress.status === 'active' && !parallaxContract && parallaxRequiredLevel !== null && profile.level < parallaxRequiredLevel && <span>NEXT PHASE LOCK // reach LV{parallaxRequiredLevel}. Standard, Directive, Command Trace, and other available contracts continue advancing operator XP.</span>}{parallaxProgress.status === 'active' && !parallaxContract && !parallaxChoice && parallaxRequiredLevel === null && <span>CAMPAIGN STATE HOLD // no closing operation is available for the current Parallax state.</span>}{parallaxProgress.status === 'complete' && <span>{parallaxProgress.choiceA === 'expose-route' ? 'Parallax Debt complete // the route is independently reproducible and its local support stack is disrupted.' : 'Parallax Debt complete // Quiet Signal retains covert access to the live private route.'}</span>}</div>{parallaxFindings.length > 0 && <div className="campaign-evidence-list">{parallaxFindings.map((entry, index) => <span key={entry}>{String(index + 1).padStart(2, '0')} // {entry}</span>)}</div>}</article>
