...
export default function ShipHub({ profile, campaign, contracts, operations, operationsStatus, operationsError, telemetrySharing, selectedContractId, statusMessage, onSelectContract, onDeploy, onOpenBuild, onCampaignChange }: Props) {
  ...
  return <main className={`ship-hub area-${primaryArea}`}>
    ...
    {tab === 'ship' && <section className="ship-systems-panel"><div className="ship-systems-intro"><div><span className="card-kicker">QUIET SIGNAL // SYSTEMS</span><h2>Upgrade what changes the next deployment.</h2><p>Ship Systems 2.0 preserves every installed prototype tier while exposing the six-tier dependency route, resource curve, and access gates. Reactor, Vector Drive, Armor Locker, and Long-Baseline Sensors now preview their commissioned Tier 3–6 combat payoff; support-system tiers remain mapped for the next wave.</p></div><button onClick={onOpenBuild}>Open Build</button></div>{upgradeGroups.map(([area, upgrades]) => <section key={area} className="upgrade-group"><h2>{area}</h2><div className="upgrade-grid">{upgrades.map(upgrade => { const status = getShipUpgradeStatus(campaign, upgrade.id)!; const level = status.level; const cost = getUpgradeCost(campaign, upgrade.id); const nextTier = status.nextTier; const currentBenefit = level > 0 ? upgrade.benefits[level - 1] : 'No major tier installed yet.'; const buttonLabel = cost ? `Upgrade // ${costText(cost)}` : !nextTier ? 'System fully realized' : status.implementationLocked ? `Tier ${nextTier.tier} blueprint queued` : `Tier ${nextTier.tier} gated`; return <article key={upgrade.id} className="upgrade-card"><div><small>{upgrade.name.toUpperCase()}</small><b>TIER {level} / {SHIP_SYSTEM_MAX_TIER}</b></div><p>{upgrade.description}</p><strong>{currentBenefit}</strong>{nextTier && <div className="upgrade-plan"><small>NEXT // TIER {nextTier.tier}</small><span>{nextTier.benefit}</span>{nextTier.gates.length > 0 && <em>{status.gateFailures.length > 0 ? `Gates: ${status.gateFailures.map(gate => gate.label).join(' · ')}` : 'All access gates satisfied'}</em>}</div>}<button disabled={!cost} onClick={() => buy(upgrade.id)}>{buttonLabel}</button></article>; })}</div></section>)}</section>}
