...
function Delta({ label, current, candidate, lowerIsBetter = false }: { label: string; current: number; candidate: number; lowerIsBetter?: boolean }) {
  ...
  const sign = delta > 0 ? '+' : '';
  return <div className={`compare-line ${favorable ? 'better' : unfavorable ? 'worse' : ''}`}><span>{label}</span><b>{candidate.toFixed(label === 'MAG' ? 0 : 1)}</b><small>{Math.abs(delta) < 0.001 ? '—' : `${sign}${delta.toFixed(label === 'MAG' ? 0 : 1)}`}</small></div>;
}
...
function GearComparison({ profile, item }: { profile: PlayerProfile; item: Item }) {
  ...
  return (
    <>
      <div className="compare-head"><div><small>CURRENT</small><b>{equipped?.name ?? 'Empty slot'}</b><span>{summary.current}</span></div><div><small>CANDIDATE</small><b>{item.name}</b><span>{summary.candidate}</span></div></div>
      {weaponSlot ? (() => {
      ...
      {item.faction && <div className={`faction-signature faction-${item.faction}`}><b>FACTION FRAME // {factionLabel(item.faction).toUpperCase()}</b><span>Counts toward the {factionSetDefinition(item.faction).setName} 2-piece and 4-piece doctrine bonuses.</span></div>}
...
export default function Armory({ profile, campaign, newLootIds, onProfileChange, onCampaignChange, onClose }: Props) {
  ...
  const [message, setMessage] = useState(newLootIds.length > 0 ? `${newLootIds.length} recovered equipment packages added to ship storage.` : 'Build changes save automatically on this device.');
