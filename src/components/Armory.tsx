...
type Props = {
  ...
  onOpenGuide: (section: GuideSectionId) => void;
  guideReturnFocus?: GuideSectionId | null;
  ...
  return (
    <section className="reconstruction-bench" data-crafting-surface="true">
      ...
      <section className="crafting-rules-contract" aria-label="Crafting rules and legal modifier pool">
        ...
        <ProgressiveDisclosure triggerLabel="How Reconstruction rules work" eyebrow="Reconstruction help" heading="Reconstruction rules, materials & control tiers" className="crafting-help-sheet"><p>The reusable rules for material tiers, crafting verbs, stability, protected control, and build integration now live in Intel → Guide. Exact costs, legal pools, current grade ceilings, risk, and blockers stay on this bench.</p></ProgressiveDisclosure>
...
export default function Armory({ profile, campaign, newLootIds, onProfileChange, onCampaignChange, onOpenGuide, guideReturnFocus, onGuideFocusRestored, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      const target = document.querySelector<HTMLButtonElement>('button[data-guide-link="' + guideReturnFocus + '"]');
      target?.focus();
  ...
  return <main ref={buildRef} className="build-bay iv-view">
    ...
    {tab === 'protocols' && <section className="protocol-panel">
      <div className="section-copy"><h2>Class Skills</h2><p>Choose Standard, a Lens, or a class Evolution for each skill. Immediate tradeoffs stay on the option; level and capstone requirements appear beside the action.</p><ProgressiveDisclosure triggerLabel="How Skills progression works" eyebrow="Skills help" heading="Class Skills progression"><p>The reusable Class Skill → Weapon Family → Lens/Evolution → Specialization/Capstone hierarchy now lives in Intel → Guide. This screen keeps each live option, tradeoff, level gate, and capstone state beside the action.</p></ProgressiveDisclosure><GuideLink section="builds-progression" label="Builds & Progression" onOpenGuide={onOpenGuide} /></div>
    ...
    {tab === 'settings' && <section className="settings-panel"><div className="section-copy"><h2>Graphics, Accessibility, Controls & Feedback</h2><p>Graphics quality, accessibility, combat assistance, audio, and feedback preferences save with the local operator profile. Each control keeps its immediate effect below.</p><GuideLink section="accessibility-settings" label="Accessibility / Settings" onOpenGuide={onOpenGuide} /></div><label><span><b>Graphics quality</b><small>Adaptive balances presentation against live frame pressure. Flagship starts with full shadows, reflections, VFX and texture sampling; Performance starts at the lowest-cost presentation tier. Combat mechanics and critical tells stay unchanged.</small></span><select aria-label="Graphics quality" value={profile.settings.graphicsQuality} onChange={event => onProfileChange(setProfileSettings(profile, { graphicsQuality: event.target.value as PlayerProfile['settings']['graphicsQuality'] }))}><option value="adaptive">Adaptive</option><option value="flagship">Flagship</option><option value="performance">Performance</option></select></label><label><span><b>Interface text size</b><small>Large scales the interface typography for easier reading while preserving responsive layouts.</small></span><select aria-label="Interface text size" value={profile.settings.textScale} onChange={event => onProfileChange(setProfileSettings(profile, { textScale: event.target.value as PlayerProfile['settings']['textScale'] }))}><option value="default">Default</option><option value="large">Large</option></select></label><label><span><b>High contrast</b><small>Strengthens text, borders, surfaces, and focus cues without removing rarity shape coding.</small></span><input aria-label="High contrast" type="checkbox" checked={profile.settings.contrast === 'high'} onChange={event => onProfileChange(setProfileSettings(profile, { contrast: event.target.checked ? 'high' : 'standard' }))} /></label><label><span><b>Reduce motion</b><small>Disables interface transitions and combat camera shake while leaving gameplay timing unchanged.</small></span><input aria-label="Reduce motion" type="checkbox" checked={profile.settings.reducedMotion} onChange={event => onProfileChange(setProfileSettings(profile, { reducedMotion: event.target.checked }))} /></label><label><span><b>Touch aim assistance</b><small>Sets how readily assisted FIRE acquires a nearby visible threat.</small></span><select aria-label="Touch aim assistance" value={profile.settings.aimAssist} onChange={event => onProfileChange(setProfileSettings(profile, { aimAssist: event.target.value as PlayerProfile['settings']['aimAssist'] }))}><option value="light">Light</option><option value="balanced">Balanced</option></select></label><label><span><b>Assisted fire tracking</b><small>When on, holding FIRE tracks a nearby threat. Turn it off for fully manual touch aiming.</small></span><input aria-label="Assisted fire tracking" type="checkbox" checked={profile.settings.rightStickFire} onChange={event => onProfileChange(setProfileSettings(profile, { rightStickFire: event.target.checked }))} /></label><label><span><b>Screen shake</b><small>Enables recoil, impact and damage camera response on the combat canvas; the HUD never moves.</small></span><input aria-label="Screen shake" type="checkbox" checked={profile.settings.screenShake} onChange={event => onProfileChange(setProfileSettings(profile, { screenShake: event.target.checked }))} /></label><label><span><b>Effect intensity</b><small>Reduced lowers secondary VFX and scales combat camera/haptic intensity without changing simulation rules.</small></span><select aria-label="Effect intensity" value={profile.settings.effectIntensity} onChange={event => onProfileChange(setProfileSettings(profile, { effectIntensity: event.target.value as PlayerProfile['settings']['effectIntensity'] }))}><option value="full">Full</option><option value="reduced">Reduced</option></select></label><label><span><b>Combat effects volume</b><small>Weapons, impacts, breaches, abilities and enemy cues.</small></span><input aria-label="Combat effects volume" type="range" min="0" max="1" step="0.05" value={profile.settings.effectsVolume} onChange={event => onProfileChange(setProfileSettings(profile, { effectsVolume: Number(event.target.value) }))} /></label><label><span><b>Interface volume</b><small>Ship controls and recovery confirmation.</small></span><input aria-label="Interface volume" type="range" min="0" max="1" step="0.05" value={profile.settings.uiVolume} onChange={event => onProfileChange(setProfileSettings(profile, { uiVolume: Number(event.target.value) }))} /></label><label><span><b>Mobile haptics</b><small>Synchronized vibration for weapon recoil, impacts, damage, dodges and breaches when supported.</small></span><input aria-label="Mobile haptics" type="checkbox" checked={profile.settings.haptics} onChange={event => onProfileChange(setProfileSettings(profile, { haptics: event.target.checked }))} /></label><label><span><b>Share anonymous run telemetry</b><small>Uploads anonymous balance telemetry and one-second position checkpoints after extractions or failed attempts. No account or device identifier is stored with a run.</small></span><input aria-label="Share anonymous run telemetry" type="checkbox" checked={profile.settings.telemetrySharing} onChange={event => onProfileChange(setProfileSettings(profile, { telemetrySharing: event.target.checked }))} /></label><div className="control-reference"><div><b>Control reference</b><span>Desktop, mobile, controller, targeting, and combat-reading guidance now live in Intel → Guide. Replay remains local because it changes the next deployment.</span></div><GuideLink section="combat-controls" label="Combat & Controls" onOpenGuide={onOpenGuide} /><button onClick={() => { onProfileChange(setProfileSettings(profile, { tutorialComplete: false })); setMessage('Field tutorial will replay on the next deployment.'); }}>Replay field tutorial</button></div></section>}
  </main>;
}
