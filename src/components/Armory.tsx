...
import { normalizeOperatorNetworkState, operatorNetworkRouteToNode } from '../game/operatorNetwork';
...
function ReconstructionBench({ item, campaign, lockedFamily, onLockFamily, onRun }: { item: Item; campaign: CampaignState; lockedFamily: ModifierFamily; onLockFamily: (family: ModifierFamily) => void; onRun: (action: ReconstructionAction) => void }) {
  ...
  return (
    <section className="reconstruction-bench">
      ...
      <div className="bench-frame"><div><b>FRAME QUALITY // {item.equipmentQuality ?? 0}/{reconstructionQualityCap(fabrication)}</b><span>{item.frameImplicit}</span><small>BASE FRAME ONLY // Improves the inherent frame property; explicit modifier grades and Augments do not scale with quality.</small></div><button onClick={() => onRun(qualityAction)}>Improve +2<small>{costLabel(reconstructionCost(item, qualityAction, fabrication))}</small></button></div>
...
export default function Armory({ profile, campaign, newLootIds, onProfileChange, onCampaignChange, onClose }: Props) {
  ...
  const allocatedNetworkNotables = progressionNodes.filter(node => node.kind === 'notable' && profile.allocatedNodes.includes(node.id)).length;
