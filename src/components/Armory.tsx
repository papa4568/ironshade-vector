...
export default function Armory({ profile, campaign, newLootIds, onProfileChange, onCampaignChange, onOpenGuide, guideReturnFocus, onGuideFocusRestored, onClose }: Props) {
  ...
  const [wideInspectorPortal, setWideInspectorPortal] = useState(false);
  useEffect(() => {
    const wideViewport = window.matchMedia('(min-width: 1101px)');
    const coarsePointer = window.matchMedia('(pointer: coarse)');
    const syncInspectorPresentation = () => setWideInspectorPortal(wideViewport.matches && !coarsePointer.matches);
    syncInspectorPresentation();
    wideViewport.addEventListener('change', syncInspectorPresentation);
    coarsePointer.addEventListener('change', syncInspectorPresentation);
    return () => {
      wideViewport.removeEventListener('change', syncInspectorPresentation);
      coarsePointer.removeEventListener('change', syncInspectorPresentation);
    };
  }, []);
  useEffect(() => {
