...
import { compareRarity, rarityClassToken, rarityDefinition, rarityOrder } from '../game/rarity';
...
function rarityCue(item: Item) { return rarityDefinition(item.rarity).cue; }
...
function RarityLegend() {
  return <div className="rarity-contract-legend" aria-label="Equipment rarity guide">{rarityOrder.map(rarity => { const definition = rarityDefinition(rarity); return <div key={rarity} className={rarityClassToken(rarity)}><span className="rarity-shape" aria-hidden="true">{definition.icon}</span><div><b>{rarity.toUpperCase()} · {definition.cue}</b><small>{definition.meaning}</small></div></div>; })}</div>;
}
...
export default function Armory({ profile, campaign, newLootIds, onProfileChange, onCampaignChange, onClose }: Props) {
  ...
  return <main ref={buildRef} className="build-bay">
    ...
    {tab === 'network' && <section className="network-panel">
      <div className="section-copy"><h2>Operator Class & Progression</h2><p>Your class establishes an early combat identity and gear resonance path. Every progression branch, weapon family, and Skill Lens remains available; class choice changes synergy rather than permission.</p></div>
      ...
      <section className={`specialization-panel ${profile.level < 15 ? 'locked' : ''}`}>
        <header><div><small>LV15+ // {operatorClassDefinition.name.toUpperCase()} SPECIALIZATIONS</small><b>Choose a deep class path with an explicit tradeoff</b><span>Specializations deepen your active class while the wider progression tree and equipment system stay open. Switching operator class clears an incompatible specialization but never rewrites allocated progression nodes or equipment.</span></div>{profile.specialization && profile.level >= 15 && <button onClick={() => { onProfileChange(setSpecialization(profile, null)); setMessage('Specialization cleared; class, Network allocations, and equipment are unchanged.'); }}>Clear specialization</button>}</header>
