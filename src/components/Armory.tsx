...
function GearComparison({ profile, item, fabrication }: { profile: PlayerProfile; item: Item; fabrication: number }) {
  ...
  const compatibleHardware = compatibleAugments(item);
  const equipReady = item.levelRequirement <= profile.level;
    ...
    <>
      <div className="gear-summary-grid">
        ...
        <section className="gear-identity-grid" aria-label="Equipment identity and compatibility">
          ...
          <div className={equipReady ? 'compatible' : 'locked'}><small>COMPATIBILITY</small><b>{equipReady ? 'EQUIP NOW' : `REQUIRES LV ${item.levelRequirement}`}</b><span>{classMatched ? `${activeClass.name} resonance active.` : `Usable by ${activeClass.name}; no class lock.`}</span></div>
        </section>
        {buildChangingEffects.length > 0 && <section className="build-change-panel"><header><small>BUILD-CHANGING EFFECTS</small><b>{buildChangingEffects.length} detected</b></header>{buildChangingEffects.map(effect => <div key={effect.label}><b>{effect.label}</b><span>{effect.detail}</span></div>)}</section>}
