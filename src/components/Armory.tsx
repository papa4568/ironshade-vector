  ...
  return <main ref={buildRef} className="build-bay">
    ...
    {tab === 'network' && <section className="network-panel">
      ...
      <section className="operator-class-panel" aria-label="Operator classes">
        ...
        <div className="operator-class-grid">{operatorClassDefinitions.map(definition => { const active = definition.id === operatorClass; const resonance = gearResonanceForProfile(profile, definition.id); return <button key={definition.id} className={active ? 'selected' : ''} aria-pressed={active} onClick={() => applyResult(setOperatorClass(profile, definition.id))}><small>{definition.identity}</small><b>{definition.name}</b><span>{definition.trait}</span><em>{definition.branchAffinities.join(' + ')} affinity</em><strong>{resonance.count}/6 matching equipped frames</strong></button>; })}</div>
    ...
    {tab === 'protocols' && <section className="protocol-panel"><div className="section-copy"><h2>Ability Protocol Lenses</h2><p>Each ability accepts one lens. Lenses are class-agnostic supports: every operator class can use every lens, while your class, Network and equipment determine how strongly the resulting behavior scales.</p></div><div className="protocol-grid">{(['mag', 'mark', 'arc'] as AbilityId[]).map(ability => <article key={ability}><h3>{abilityLabels[ability]}</h3><button className={!profile.abilityMods[ability] ? 'selected' : ''} onClick={() => onProfileChange(setAbilityMod(profile, ability, null))}><b>Standard Protocol</b><span>Original Part 2 behavior with no additional tradeoff.</span></button>{abilityMods.filter(mod => mod.ability === ability).map(mod => <button key={mod.id} className={profile.abilityMods[ability] === mod.id ? 'selected' : ''} onClick={() => { onProfileChange(setAbilityMod(profile, ability, mod.id)); setMessage(`${mod.name} installed on ${abilityLabels[ability]}.`); }}><b>{mod.name}</b><span>{mod.description}</span><small>TRADEOFF // {mod.tradeoff}</small></button>)}</article>)}</div></section>}
