...
import { buildIdentity, deriveCombatBuild, xpProgress, type PlayerProfile } from '../game/meta';
import { abilityMeta, createSimulation, type WeaponId } from '../game/sim';
...
export default function PlayerStatsPanel({ profile, campaign }: Props) {
  ...
  return <section className="stats-panel" aria-label="Player statistics">
    ...
    <section className="stats-section"><div className="stats-section-heading"><div><small>SURVIVAL + MOVEMENT</small><h3>Operator frame</h3></div><p>Final values after equipped gear, progression and ship bonuses.</p></div><div className="player-stat-grid">{survival.map(stat => <StatCard key={stat.label} stat={stat} />)}</div></section>
    <section className="stats-section"><div className="stats-section-heading"><div><small>WEAPONS</small><h3>Final weapon telemetry</h3></div><p>Damage is per projectile. Burst DPS is an estimate before armor, misses, reloads or heat downtime.</p></div><div className="weapon-stat-grid">{(Object.keys(sim.weapons) as WeaponId[]).map(id => { const weapon = sim.weapons[id]; const burst = weapon.damage * weapon.pellets * weapon.rate * weapon.healthMultiplier; return <article className="weapon-stat-card" key={id}><header><small>{weaponLabels[id]}</small><b>{weapon.shortName}</b></header><div className="weapon-stat-values"><span><small>DMG / PROJECTILE</small><b>{number(weapon.damage)}</b></span><span><small>BURST DPS</small><b>{number(burst, 0)}</b></span><span><small>PENETRATION</small><b>{number(weapon.penetration, 0)}</b></span><span><small>ARMOR FACTOR</small><b>{number(weapon.armorDamage, 2)}×</b></span><span><small>MAGAZINE</small><b>{weapon.magazine}</b></span><span><small>RELOAD</small><b>{number(weapon.reloadSeconds, 2)}s</b></span><span><small>HEAT / SHOT</small><b>{Math.round(weapon.heatPerShot * 100)}%</b></span><span><small>RECOIL</small><b>{number(weapon.recoil, 0)}</b></span></div><details><summary>What these numbers mean</summary><p><b>Penetration</b> helps shots punch through armor and hard targets. <b>Armor factor</b> multiplies damage dealt specifically to armor. <b>Recoil</b> changes weapon push and handling. Heat accumulation can force venting or downtime.</p></details></article>; })}</div></section>
    <section className="stats-section"><div className="stats-section-heading"><div><small>ABILITIES</small><h3>MAG / MARK / ARC</h3></div><p>Power, capacitor cost and cooldown include your current equipment and build modifiers.</p></div><div className="ability-stat-grid">{abilityMeta.map((ability, index) => { const mod = build.abilities[index]; return <article key={ability.shortName}><small>{ability.shortName}</small><b>{ability.name}</b><div><span>Power <strong>{pct(mod.powerMul)}</strong></span><span>Cost <strong>{number(ability.cost * mod.costMul, 0)} CAP</strong></span><span>Cooldown <strong>{number(ability.cooldown * mod.cooldownMul, 2)}s</strong></span></div></article>; })}</div></section>
