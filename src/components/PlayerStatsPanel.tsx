...
import { buildIdentity, capstoneInteractionFor, deriveCombatBuild, gearResonanceForProfile, operatorClassDefinitions, operatorClassForProfile, specializationDefinitions, specializationGearSynergyForProfile, xpProgress, type PlayerProfile } from '../game/meta';
...
export default function PlayerStatsPanel({ profile, campaign }: Props) {
  ...
  return <section className="stats-panel" aria-label="Player statistics">
    ...
    <section className="stats-section mechanics-section"><div className="stats-section-heading"><div><small>BUILD RULES</small><h3>Active behavior changes</h3></div><p>Mechanics that change how attacks or abilities behave, beyond ordinary numerical bonuses.</p></div>{activeMechanics.length || build.singularTraits.length || activeGearSynergy?.active ? <div className="mechanic-chip-grid">{activeGearSynergy?.active && <span>Gear Link // {activeGearSynergy.definition.name}</span>}{activeMechanics.map(label => <span key={label}>{label}</span>)}{build.singularTraits.map(trait => <span key={trait}>Singular // {trait.replace(/([A-Z])/g, ' $1').trim()}</span>)}</div> : <div className="stats-empty"><b>No rule-changing mechanics active.</b><span>Acquire and equip mechanical modifiers or Singular gear to add behavior-changing effects.</span></div>}</section>
  </section>;
}
