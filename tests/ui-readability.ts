...
assert(shipHub.includes("overview: 'Tasking'") && shipHub.includes("const pageTitle = tab === 'overview' ? 'Tasking'"), 'Command overview still repeats the Command label instead of using a distinct tasking page title.');
assert(combat.includes('visibleConsumables') && combat.includes("consumableDefinitions.filter(item => consumableStock[item.id] > 0)") && mobileCombatCss.includes('.combat-alert'), 'Mobile combat still reserves persistent HUD space for empty consumables or oversized mission alerts.');
