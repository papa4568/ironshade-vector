...
assert(androidSmokeShell.includes('--activity-reorder-to-front') && androidSmokeShell.includes('RESUME_PID') && androidSmokeShell.includes('ANDROID_LIFECYCLE_PROCESS_RECLAIM') && androidSmokeShell.includes('ANDROID_RESUME_PROCESS_MODE="$RESUME_PROCESS_MODE"'), 'Android lifecycle smoke must foreground the existing activity and distinguish same-process resume from OS process reclaim before validating combat recovery');
