...
assert(armory.includes('function RarityText') && armory.includes('definition.icon') && armory.includes('rarityDisplayLabel(rarity)'), 'Build gear UI must keep non-color rarity shape/label cues on item decisions.');
assert(guide.includes("id: 'rarity'") && guide.includes('Rarity language') && guide.includes('points: rarityPoints'), 'P19-C Intel Guide must own the canonical reusable rarity explanation.');
assert(!armory.includes('rarity-contract-legend') && !armory.includes('Equipment rarity guide'), 'P19-C must not duplicate the full rarity legend inside Build.');
...
assert(combat.includes('data-tutorial-step={tutorialStep}') && androidSmoke.includes("getAttribute('data-tutorial-step')"), 'P19-A Android input smoke must observe tutorial progress independently of shared transient-alert presentation priority.');
...
assert(armory.includes('const equipRequirement: RequirementPresentation') && armory.includes("label: 'Class-family armament locked'") && armory.includes('Requires operator level') && armory.includes('<ActionRequirement presentation={equipRequirement} />'), 'P18-E gear inspector must explain ready/active/blocked equip state, class-family locks, and level requirements before deep detail.');
assert(armory.includes('<ProgressiveDisclosure triggerLabel="Details"') && armory.includes('gear-details-sheet') && armory.includes('EXPLICIT MODIFIERS') && armory.includes('AUGMENTS') && armory.includes('GENERATED BUILD LINKS') && armory.includes('PROVENANCE / REGISTRY') && !armory.includes('<details className="gear-deep-details"'), 'P18-E expert modifier, Augment, build-link, and provenance data must live in the shared Details sheet without information loss.');
assert(armory.includes('KEY LOADOUT CHANGES') && armory.includes('impact-grid--quick') && armory.includes('BUILD-CHANGING EFFECTS') && armory.includes('gear-build-change-quick'), 'P18-E default inspector is missing concise comparison stats or build-changing rules.');
assert(equipmentCss.includes('P18-E // Gear inspector quick-read + shared expert Details sheet') && equipmentCss.includes('.gear-summary-grid--quick > .iv-requirement') && equipmentCss.includes('.gear-summary-grid--quick > .iv-disclosure-trigger') && equipmentCss.includes('.gear-details-sheet .impact-grid--full'), 'P18-E gear quick-read/shared Details presentation is missing responsive decision-first styling.');
assert(androidSmoke.includes('ANDROID_P18_GEAR_INSPECTOR_PASS') && androidSmoke.includes('p18GearStorageCandidate') && androidSmoke.includes('p18GearRestoreCard'), 'P18-E Android smoke must cover storage compare/details plus support-slot unequip/equip restoration.');
assert(armory.includes('function itemEquipRequirement') && armory.includes('function conciseItemDelta') && armory.includes('inventory-card-decision') && armory.includes('inventory-card-delta') && armory.includes('data-requirement-state={requirement.state}') && !armory.includes('item-depth-line'), 'P19-D storage cards must expose item/rarity/slot/effect plus explicit equip state and concise comparison without expert metadata stacks.');
assert(armory.includes('GuideLink section="equipment-rarity"') && armory.includes('gear-details-sheet') && armory.includes('PROVENANCE / REGISTRY') && armory.includes('SOURCE / PROVENANCE') && armory.includes('FRAME QUALITY') && armory.includes('EXPLICIT MODIFIERS') && armory.includes('AUGMENTS'), 'P19-D removed card metadata must remain reachable through the inspector/Details flow with durable teaching routed to the Equipment & Rarity Guide.');
assert(css.includes('P19-D // Armory inventory cards become decision-first') && css.includes('.inventory-card-decision') && css.includes('.inventory-card-delta') && !css.includes('.inventory-card .item-depth-line'), 'P19-D readability styles must remove the tiny expert metadata line and provide readable decision/effect/delta text.');
assert(equipmentCss.includes('P19-D // Compact-phone Armory card composition') && equipmentCss.includes('grid-template-columns: repeat(4, minmax(0, 1fr))') && equipmentCss.includes('grid-template-columns: repeat(2, minmax(0, 1fr))') && equipmentCss.includes('font-size:14px') && progressionCss.includes('P19-D // Legacy card cascade narrowed') && !progressionCss.includes('.inventory-card small {') && !progressionCss.includes('.inventory-card span {') && !progressionCss.includes('.equipped-card small,.equipped-card span'), 'P19-D compact Armory cards must recompose and the lazy legacy stylesheet must not shrink decision text.');
assert(androidSmoke.includes('ANDROID_P19_ARMORY_CARD_PASS') && androidSmoke.includes('p19ArmoryCardScan'), 'P19-D Android smoke must verify decision-first storage/equipped cards before inspector and Guide drilldown.');
assert(armory.includes('data-management-surface="crafting"') && armory.includes('data-management-surface="progression"') && armory.includes('data-management-surface="skills"'), 'P19-E must mark all three Build management surfaces for hierarchy/readability verification.');
assert(armory.includes('GuideLink section="crafting"') && armory.includes('GuideLink section="builds-progression"') && !armory.includes('How Reconstruction rules work') && !armory.includes('How Skills progression works'), 'P19-E durable management teaching must resolve to the canonical Guide instead of duplicate how-it-works disclosures.');
assert(armory.includes('triggerLabel="View planned build math"') && armory.includes('triggerLabel="View current skill path"') && armory.includes('craftingPreviewRequirement') && armory.includes('focusedAllocationRequirement') && armory.includes('skillEvolutionRequirement'), 'P19-E must keep decision-specific previews in shared Details while preserving P18-F requirement-state implementations.');
assert(armory.includes('PLAN PREVIEW // No points spent') && armory.includes('TRADEOFF //') && armory.includes('costLabel(reconstructionCost') && armory.includes('Why blocked:') === false, 'P19-E source must retain immediate plan/cost/tradeoff decisions while blocker copy remains generated by ActionRequirement rather than duplicated prose.');
assert(classBuildCss.includes('P19-E // Compact-phone management hierarchy') && classBuildCss.includes('--p19-management-secondary: max(12px, .75rem)') && classBuildCss.includes('--p19-management-decision: max(14px, .875rem)') && classBuildCss.includes('max-height: 560px') && classBuildCss.includes('.protocol-panel .skill-path-grid'), 'P19-E compact-landscape management text floor, scalable Large mode, or Skills reflow is missing.');
assert(guide.includes("id: 'builds-progression'") && guide.includes("id: 'crafting'") && guide.includes('Operator Network routing') && guide.includes('Skills, Evolutions & specialization') && guide.includes('Control, stability & volatile work'), 'P19-E canonical Guide is missing durable Progression, Skills, or Crafting teaching.');
assert(androidSmoke.includes('ANDROID_P19_MANAGEMENT_HIERARCHY_PASS') && androidSmoke.includes('ANDROID_P18F_CRAFTING_REQUIREMENT_PASS') && androidSmoke.includes('ANDROID_P18F_PROGRESSION_REQUIREMENT_PASS') && androidSmoke.includes('ANDROID_P18F_SKILLS_REQUIREMENT_PASS') && androidSmoke.includes('__p19ManagementGamepad') && androidSmoke.includes('Crafting Guide deep-link'), 'P19-E Android smoke must close P18-F verification and cover compact hierarchy, controller navigation, and Guide deep-link behavior.');
...
assert(shipHub.includes("operations: ['contracts', 'operations']") && shipHub.includes("intel: ['campaign', 'stories', 'factions', 'guide']") && shipHub.includes("stats: 'Stats'") && shipHub.includes("operations: 'Uplink'"), 'Primary information architecture still mixes campaign/intel with deployment or uses ambiguous Stats/Uplink labels.');
...
assert(browserSmoke.includes('BROWSER_P19_COMMAND_NAV_PASS') && browserSmoke.includes("result.composition !== 'dock'") && browserSmoke.includes('primaryCount !== 5'), 'Browser E2E is missing P19-B compact-dock/wide-rail layout coverage.');
...
assert(armory.includes('className="armory-item-modal"') && armory.includes('role="dialog"') && armory.includes('aria-modal="true"') && !armory.includes('wideInspectorPortal'), 'Armory item cards must open one dedicated modal window across viewport/pointer modes.');
assert(equipmentCss.includes('.armory-item-modal > .item-inspector.open') && equipmentCss.includes('place-items: center') && equipmentCss.includes('backdrop-filter: blur(5px)'), 'Dedicated Armory item modal styling or backdrop is missing.');
assert(browserSmoke.includes('BROWSER_P19_ITEM_MODAL_PASS') && androidSmoke.includes('ANDROID_P19_ITEM_MODAL_PASS'), 'Dedicated Armory item modal is missing Browser or Android runtime verification.');
assert(armory.includes('OPERATOR CLASS // FIELD DOCTRINE') && armory.includes('class-resonance-strip') && armory.includes('availableSpecializations') && armory.includes('GuideLink section="builds-progression"') && !armory.includes('class-freedom-note'), 'Operator class selection or class-specific specialization routes are missing, or durable class teaching was not moved to the Builds & Progression Guide.');
assert(armory.includes('function itemBuildFitLabel') && armory.includes('itemBuildAffinities(item).includes(activeClassId)') && armory.includes('SPECIALIZATION LINK') && armory.includes('ARSENAL') && armory.includes('inventory-card-decision'), 'Equipment UI does not surface class/build fit on recovered gear.');
...
assert(armory.includes('New combat recoveries roll only your owned weapon family') && armory.includes('section="equipment-rarity"') && armory.includes('Class-locked armament') && armory.includes("!isItemClassCompatible(profile, item)") && guide.includes('Vanguard owns Breacher:') && guide.includes('Vector owns Rail Lance:') && guide.includes('Systems owns Carbine:') && !armory.includes('Weapon family ownership tutorial') && !armory.includes('weaponFamilyLessons') && !armory.includes('Usable by ${activeClass.name}; no class lock.') && !armory.includes('Every progression branch, weapon family, and Skill Lens remains available'), 'P8-E Build UI must enforce arsenal ownership locally while routing durable teaching to the canonical Equipment & Rarity Guide.');
...
assert(armory.includes('aria-label="Class skill hierarchy"') && armory.includes('skill-hierarchy-grid') && armory.includes('triggerLabel="View current skill path"') && armory.includes('SHARED LENSES') && armory.includes('CLASS EVOLUTIONS') && armory.includes('activeSkillBuild.sources.length'), 'P8-H Skills UI does not preserve the class → family → lens/evolution → specialization/capstone hierarchy through decision-first options and shared Details.');
...
assert(browserSmoke.includes('BROWSER_SKILL_HIERARCHY_PASS') && browserSmoke.includes('P19-E decision-first skill hierarchy') && browserSmoke.includes('View current skill path') && browserSmoke.includes('SHARED LENSES') && browserSmoke.includes('CLASS EVOLUTIONS'), 'Browser E2E does not exercise the P8-H/P19-E decision-first Skills hierarchy and shared Details flow.');
...
assert(androidSmoke.includes('ANDROID_SKILL_HIERARCHY_PASS') && androidSmoke.includes('P19-E decision-first skill hierarchy') && androidSmoke.includes('View current skill path') && androidSmoke.includes('SHARED LENSES') && androidSmoke.includes('CLASS EVOLUTIONS'), 'Android smoke does not exercise the P8-H/P19-E touch Skills hierarchy and shared Details flow.');
...
assert(armory.includes('FRAME / IMPLICIT') && armory.includes('EXPLICIT MODIFIERS') && armory.includes('AUGMENTS') && armory.includes('GENERATED BUILD LINKS') && armory.includes('FULL STAT COMPARISON') && armory.includes('KEY LOADOUT CHANGES'), 'P8.5-J item inspection information must preserve base/implicit, explicit mods, Augments, build links, and equipped impact while P18-E keeps the decision summary first.');
