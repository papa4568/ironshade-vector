...
assert(armory.includes('P9-F // RECALIBRATION') && armory.includes('runNetworkRefund') && armory.includes('runNetworkRebuild') && armory.includes('operatorNetworkRespecCreditCost'), 'P9-F progression UI must expose safe node refunds, full rebuilds, and visible recalibration costs.');\nassert(armory.includes('P10-A // CRAFTING CONTRACT') && armory.includes('craftingRulesForItem') && armory.includes('LEGAL POOL') && armory.includes('Base frame decides this list.'), 'P10-A Crafting UI must expose the canonical rule contract and visible base-owned legal pools.');\nassert(classBuildsCss.includes('P10-A // Crafting rules foundation') && classBuildsCss.includes('.crafting-rules-contract') && classBuildsCss.includes('.crafting-pool-entry.legal'), 'P10-A Crafting presentation is missing legal/locked pool readability.');
